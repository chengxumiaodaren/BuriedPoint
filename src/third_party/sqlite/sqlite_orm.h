...
/*
 *  This header makes central C++ functionality on which sqlite_orm depends
...
#include <cstddef>  //  sqlite_orm is using size_t, ptrdiff_t, nullptr_t everywhere, pull it in early
    ...
    // earlier clang versions didn't make nullptr_t available in the global
    // namespace via stddef.h, though it should have according to C++
    // documentation (see
    // https://en.cppreference.com/w/cpp/types/nullptr_t#Notes). actually it
    // should be available when including stddef.h
    using std::nullptr_t;
...
/*
 *  This header defines macros for circumventing compiler quirks on which
 * sqlite_orm depends. May amend cxx_core_features.h
 */
...
// clang has the bad habit of diagnosing missing brace-init-lists when
// constructing aggregates with base classes. This is a false positive, since
// the C++ standard is quite clear that braces for nested or base objects may be
// omitted, see
// https://en.cppreference.com/w/cpp/language/aggregate_initialization: "The
// braces around the nested initializer lists may be elided (omitted),
//  in which case as many initializer clauses as necessary are used to
//  initialize every member or element of the corresponding subaggregate, and
//  the subsequent initializer clauses are used to initialize the following
...
#define SQLITE_ORM_CLANG_SUPPRESS_MISSING_BRACES(...) \
  SQLITE_ORM_CLANG_SUPPRESS("-Wmissing-braces", __VA_ARGS__)
...
// when it tries to instantiate an expression in a requires expression, which
// results in an error, the compiler reports an error instead of dismissing the
// templated function.
#if defined(SQLITE_ORM_CONCEPTS_SUPPORTED) && \
...
template <template <class...> class Op, class... Args>
SQLITE_ORM_INLINE_VAR constexpr bool is_detected_v =
...
SQLITE_ORM_INLINE_VAR constexpr bool
    is_specialization_of_v<Primary<Types...>, Primary> = true;

template <typename Type, template <typename...> class Primary>
struct is_specialization_of
    : bool_constant<is_specialization_of_v<Type, Primary>> {};
#endif
...
using match_specialization_of =
    std::enable_if_t<polyfill::is_specialization_of_v<T, Primary>>;
...
template <template <typename...> class Op, class... Args>
using satisfies_not =
    std::enable_if_t<polyfill::negation<Op<Args...>>::value, bool>;

// enable_if for functions
template <class T, template <typename...> class Primary>
using satisfies_is_specialization_of =
    std::enable_if_t<polyfill::is_specialization_of_v<T, Primary>, bool>;
}  // namespace internal
...
class orm_error_category : public std::error_category {
 public:
  const char* name() const noexcept override final { return "ORM error"; }
  ...
  std::string message(int c) const override final {
    switch (static_cast<orm_error_code>(c)) {
      ...
      case orm_error_code::cannot_use_default_value:
        return "The statement 'INSERT INTO * DEFAULT VALUES' can be used with "
               "only one row";
...
class sqlite_error_category : public std::error_category {
 public:
  const char* name() const noexcept override final { return "SQLite error"; }

  std::string message(int c) const override final { return sqlite3_errstr(c); }
};
...
[[noreturn]] void throw_error(sqlite3* db, T&&... args) {
  throw std::system_error{sqlite_errc(sqlite3_errcode(db)),
...
// Note: char, unsigned/signed char are used for storing integer values, not
// char values.
template <class T>
struct type_printer<
    T, std::enable_if_t<polyfill::conjunction_v<
           polyfill::negation<internal::is_any_of<T, wchar_t,
#ifdef __cpp_char8_t
                                                  char8_t,
#endif
                                                  char16_t, char32_t>>,
...
template <class T>
struct type_printer<T, std::enable_if_t<std::is_floating_point<T>::value>>
    : real_printer {};

template <class T>
struct type_printer<
    T, std::enable_if_t<polyfill::disjunction_v<
           std::is_same<T, const char*>, std::is_base_of<std::string, T>,
           std::is_base_of<std::wstring, T>>>> : text_printer {};

template <class T>
struct type_printer<T, std::enable_if_t<is_std_ptr<T>::value>>
    : type_printer<typename T::element_type> {};

#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
template <class T>
struct type_printer<
    T, std::enable_if_t<polyfill::is_specialization_of_v<T, std::optional>>>
 ...
 *  1. A 'metafunction' is a class template that represents a function invocable
 * at compile-time.
 *  2. A 'metafunction class' is a certain form of metafunction representation
 * that enables higher-order metaprogramming. More precisely, it's a class with
 * a nested metafunction called "fn" Correspondingly, a metafunction class
 * invocation is defined as invocation of its nested "fn" metafunction.
 *  3. A 'metafunction operation' is an alias template that represents a
 * function whose instantiation already yields a type.
 *
 *  Conventions:
 ...
 *  - "_fn" is a suffix for a type that accepts metafunctions and turns them
 * into metafunction classes.
 *  - "higher order" denotes a metafunction that operates on another
 * metafunction (i.e. takes it as an argument).
 */
 ...
 *
 *  Implementation note: the technique of specialiazing on the inline variable
 * must come first because of older compilers having problems with the detection
 * of dependent templates [SQLITE_ORM_BROKEN_VARIADIC_PACK_EXPANSION].
 */
template <class T, class SFINAE = void>
SQLITE_ORM_INLINE_VAR constexpr bool is_metafunction_class_v = false;
template <class FnCls>
SQLITE_ORM_INLINE_VAR constexpr bool is_metafunction_class_v<
    FnCls, polyfill::void_t<indirectly_test_metafunction<FnCls::template fn>>> =
    true;

template <class T>
struct is_metafunction_class
    : polyfill::bool_constant<is_metafunction_class_v<T>> {};
 ...
 *
 *  Note: legacy compilers need an extra layer of indirection, otherwise type
 * replacement may fail if alias template `Op` has a dependent expression in it.
 */
template <template <class...> class Op, class... Args>
 ...
 *
 *  Invocation of the nested metafunction `fn` is SFINAE-friendly (detection
 * idiom). This is necessary because `fn` is a proxy to the originally quoted
 * metafunction, and the instantiation of the metafunction might be an invalid
 * expression.
 */
template <template <class...> class Fn>
struct quote_fn {
...
struct higherorder<0u> {
   ...
   */
  template <template <template <class...> class Fn, class... Args2>
            class HigherFn>
  struct quote_fn {
...
/*
 *  Metafunction class that extracts the nested metafunction of its metafunction
 * class argument, quotes the extracted metafunction and passes it on to the
 * next metafunction class (kind of the inverse of quoting).
 */
template <class FnCls>
...
struct conjunction {
  template <class... Args>
  struct fn
      : polyfill::conjunction<typename TraitFnCls::template fn<Args...>...> {};
};
...
struct disjunction {
  template <class... Args>
  struct fn
      : polyfill::disjunction<typename TraitFnCls::template fn<Args...>...> {};
};
...
/*
 *  Convenience template alias for binding arguments at the front of a
 * metafunction.
 */
...
/*
 *  Convenience template alias for binding arguments at the back of a
 * metafunction.
 */
template <template <class...> class Fn, class... Bound>
...
/*
 *  Convenience template alias for binding a metafunction at the front of a
 * higher-order metafunction.
 */
template <template <template <class...> class Fn, class... Args2>
          class HigherFn,
          template <class...> class BoundFn, class... Bound>
...
/*
 *  Trait metafunction class that checks if a type doesn't have the specified
 * trait.
 */
template <template <class...> class TraitFn>
...
/*
 *  Trait metafunction class that checks if a type is the same as the specified
 * type.
 */
template <class Type>
...
/*
 *  Trait metafunction class that checks if a type's template matches the
 * specified template (similar to `is_specialization_of`).
 */
template <template <class...> class Template>
using check_if_is_template = mpl::pass_extracted_fn_to<
...
/**
 *  Accepts any number of arguments and evaluates `type` alias as T if all
 * arguments are the same or void otherwise
 */
template <class... Args>
struct same_or_void {
  using type = void;
...
namespace internal {
/*
 *  Higher-order trait metafunction that checks whether a tuple contains a type
 * with given trait.
 */
template <template <class...> class TraitFn, class Tuple>
struct tuple_has {};
template <template <class...> class TraitFn, class... Types>
struct tuple_has<TraitFn, std::tuple<Types...>>
    : polyfill::disjunction<TraitFn<Types>...> {};

/*
 *  Trait metafunction class that checks whether a tuple contains a type with
 * given trait.
 */
template <template <class...> class TraitFn>
...
/*
 *  Trait metafunction class that checks whether a tuple doesn't contain a type
 * with given trait.
 */
template <template <class...> class TraitFn>
...
using check_if_tuple_has_type =
    mpl::bind_front_higherorder_fn<tuple_has, check_if_is_type<T>::template fn>;

/*
 *  Metafunction class that checks whether a tuple contains a given template.
 *
 *  Note: we are using 2 small tricks:
 *  1. A template template parameter can be treated like a metafunction, so we
 * can just "quote" a 'primary' template into the MPL system (e.g.
 * `std::vector`).
 *  2. This metafunction class does the opposite of the trait function
 * `is_specialization`: `is_specialization` tries to instantiate the primary
 * template template parameter using the template parameters of a template type,
 * then compares both instantiated types. Here instead, `pass_extracted_fn_to`
 * extracts the template template parameter from a template type, then compares
 * the resulting template template parameters.
 */
template <template <class...> class Primary>
using check_if_tuple_has_template =
    mpl::bind_front_higherorder_fn<tuple_has,
                                   check_if_is_template<Primary>::template fn>;
}  // namespace internal
}  // namespace sqlite_orm
...
SQLITE_ORM_CONSTEVAL size_t
first_index_sequence_value(std::index_sequence<I, Idx...>) {
  return I;
}
...
struct flatten_idxseq<std::index_sequence<As...>, std::index_sequence<Bs...>,
                      Seq...>
...
template <class Tpl, class Seq>
using tuple_from_index_sequence_t =
    typename tuple_from_index_sequence<Tpl, Seq>::type;

template <class Tpl, template <class...> class Pred,
          template <class...> class Proj, class Seq>
struct filter_tuple_sequence;

#ifndef SQLITE_ORM_BROKEN_VARIADIC_PACK_EXPANSION
template <class Tpl, template <class...> class Pred,
          template <class...> class Proj, size_t... Idx>
struct filter_tuple_sequence<Tpl, Pred, Proj, std::index_sequence<Idx...>>
    : flatten_idxseq<std::conditional_t<
          Pred<Proj<std::tuple_element_t<Idx, Tpl>>>::value,
          std::index_sequence<Idx>, std::index_sequence<>>...> {};
#else
...
template <class Tpl, template <class...> class Pred,
          template <class...> class Proj, size_t... Idx>
struct filter_tuple_sequence<Tpl, Pred, Proj, std::index_sequence<Idx...>>
    : flatten_idxseq<typename tuple_seq_single<
          Idx, Proj<std::tuple_element_t<Idx, Tpl>>, Pred>::type...> {};
#endif
...
using filter_tuple_sequence_t =
    typename filter_tuple_sequence<Tpl, Pred, Proj, Seq>::type;

template <class Tpl, template <class...> class Pred,
          template <class...> class FilterProj = polyfill::type_identity_t,
          class Seq = std::make_index_sequence<std::tuple_size<Tpl>::value>>
using filter_tuple_t = tuple_from_index_sequence_t<
    Tpl, filter_tuple_sequence_t<Tpl, Pred, FilterProj, Seq>>;

template <class Tpl, template <class...> class Pred,
          template <class...> class FilterProj = polyfill::type_identity_t>
struct count_tuple
    : std::integral_constant<
          int, filter_tuple_sequence_t<Tpl, Pred, FilterProj>::size()> {};

/*
 *  Count a tuple, picking only those elements specified in the index sequence.
 *
 *  Implementation note: must be distinct from `count_tuple` because legacy
 * compilers have problems with a default Sequence in function template
 * parameters [SQLITE_ORM_BROKEN_VARIADIC_PACK_EXPANSION].
 */
template <class Tpl, template <class...> class Pred, class Seq,
          template <class...> class FilterProj = polyfill::type_identity_t>
struct count_filtered_tuple
    : std::integral_constant<
          size_t, filter_tuple_sequence_t<Tpl, Pred, FilterProj, Seq>::size()> {
};
}  // namespace internal
}  // namespace sqlite_orm
 ...
 * pointers). Can be empty when used within `make_column` function.
 */
template <class... Cs>
struct primary_key_t : primary_key_base {
  ...
  primary_key_with_autoincrement<self> autoincrement() const { return {*this}; }
...
struct unique_base {
  operator std::string() const { return "UNIQUE"; }
};
...
struct on_update_delete_t : on_update_delete_base {
  ...
  on_update_delete_t(decltype(fk) fk_, decltype(update) update_,
  ...
  operator bool() const { return this->_action != foreign_key_action::none; }
};

template <class F>
bool operator==(const on_update_delete_t<F>& lhs,
                const on_update_delete_t<F>& rhs) {
...
struct foreign_key_t<std::tuple<Cs...>, std::tuple<Rs...>> {
  ...
  static_assert(std::tuple_size<columns_type>::value ==
                    std::tuple_size<references_type>::value,
                "Columns size must be equal to references tuple");
  static_assert(!std::is_same<target_type, void>::value,
                "All references must have the same type");
...
template <class A, class B>
bool operator==(const foreign_key_t<A, B>& lhs,
                const foreign_key_t<A, B>& rhs) {
  return lhs.columns == rhs.columns && lhs.references == rhs.references &&
...
struct foreign_key_intermediate_t {
  ...
  foreign_key_t<std::tuple<Cs...>, std::tuple<Rs...>> references(Rs... refs) {
    return {std::move(this->columns),
            std::make_tuple(std::forward<Rs>(refs)...)};
  }
};
#endif
...
struct generated_always_t : basic_generated_always {
  ...
  generated_always_t(expression_type expression_, bool full,
                     storage_type storage)
      : basic_generated_always{full, storage},
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_primary_key_v =
    is_primary_key<T>::value;

template <class T>
using is_generated_always =
    polyfill::is_specialization_of<T, generated_always_t>;

template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_generated_always_v =
    is_generated_always<T>::value;

template <class T>
using is_autoincrement = std::is_same<T, autoincrement_t>;

template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_autoincrement_v =
    is_autoincrement<T>::value;
...
struct is_primary_key_insertable
  ...
  static_assert(tuple_has<is_primary_key, constraints_type_t<T>>::value,
                "an unexpected type was passed");
};

template <class T>
using is_constraint = mpl::instantiate<
    mpl::disjunction<check_if<is_autoincrement>, check_if<is_primary_key>,
                     check_if<is_foreign_key>, check_if_is_template<unique_t>,
...
internal::generated_always_t<T> generated_always_as(T expression) {
  return {std::move(expression), true,
          internal::basic_generated_always::storage_type::not_specified};
}
...
inline internal::unique_t<> unique() { return {{}}; }

/**
 *  AUTOINCREMENT keyword. [Deprecation notice] Use
 * `primary_key().autoincrement()` instead of using this function. This function
 * will be removed in 1.9
 */
[[deprecated("Use primary_key().autoincrement()` instead")]] inline internal::
    autoincrement_t
    autoincrement() {
  return {};
}
...
inline internal::primary_key_t<> primary_key() { return {{}}; }
...
/**
 *  This is a specialization for std::shared_ptr, std::unique_ptr,
 * std::optional, which are nullable in sqlite_orm.
 */
template <class T>
struct type_is_nullable<
    ...
    : std::true_type {
  bool operator()(const T& t) const { return static_cast<bool>(t); }
};
...
struct conc_string {
  serialize_result_type serialize() const { return "||"; }
};
...
template <class L, class R>
using mod_t =
    binary_operator<L, R, mod_operator_string, arithmetic_t, negatable_t>;
 ...
 */
template <class L, class R>
using bitwise_shift_right_t = binary_operator<L, R, bitwise_shift_right_string,
                                              arithmetic_t, negatable_t>;
...
/**
 *  Public interface for - operator. Example: `select(sub(&User::age, 1));` =>
...
/**
 *  Public interface for * operator. Example: `select(mul(&User::salary, 2));`
 * => SELECT salary * 2 FROM users
 */
template <class L, class R>
...
/**
 *  Public interface for / operator. Example: `select(div(&User::salary, 3));`
 * => SELECT salary / 3 FROM users
 *  @note Please notice that ::div function already exists in pure C standard
 * library inside <cstdlib> header. If you use `using namespace sqlite_orm`
 * directive you an specify which `div` you call explicitly using  `::div` or
 * `sqlite_orm::div` statements.
 */
template <class L, class R>
...
/**
 *  Public interface for % operator. Example: `select(mod(&User::age, 5));` =>
...
struct object_field_type<F O::*>
    : std::enable_if<!std::is_function<F>::value, F> {};

// SFINAE friendly trait to get a member function pointer's field type (i.e.
// unqualified return type)
template <class T>
...
template <class T>
struct member_field_type : object_field_type<T>,
                           getter_field_type<T>,
                           setter_field_type<T> {};
 ...
 *  Column definition.
 *
 *  It is a composition of orthogonal information stored in different base
 * classes.
 */
template <class G, class S, class... Op>
struct column_t : column_identifier,
                  column_field<G, S>,
                  column_constraints<Op...> {
#ifndef SQLITE_ORM_AGGREGATE_BASES_SUPPORTED
  column_t(std::string name, G memberPointer, S setter, std::tuple<Op...> op)
      : column_identifier{std::move(name)},
        column_field<G, S>{memberPointer, setter},
        column_constraints<Op...>{std::move(op)} {}
#endif
};

template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_column_v =
    polyfill::is_specialization_of_v<T, column_t>;
...
using col_index_sequence_with_field_type =
    filter_tuple_sequence_t<Elements, check_if_is_type<F>::template fn,
                            field_type_t,
                            filter_tuple_sequence_t<Elements, is_column>>;

template <class Elements, template <class...> class TraitFn>
using col_index_sequence_with =
    filter_tuple_sequence_t<Elements, check_if_tuple_has<TraitFn>::template fn,
                            constraints_type_t,
                            filter_tuple_sequence_t<Elements, is_column>>;

template <class Elements, template <class...> class TraitFn>
using col_index_sequence_excluding = filter_tuple_sequence_t<
    Elements, check_if_tuple_has_not<TraitFn>::template fn, constraints_type_t,
    filter_tuple_sequence_t<Elements, is_column>>;
}  // namespace internal
...
template <class M, class... Op,
          internal::satisfies<std::is_member_object_pointer, M> = true>
internal::column_t<M, internal::empty_setter, Op...> make_column(
    std::string name, M m, Op... constraints) {
  static_assert(polyfill::conjunction_v<internal::is_constraint<Op>...>,
                "Incorrect constraints pack");

  SQLITE_ORM_CLANG_SUPPRESS_MISSING_BRACES(
      return {std::move(name), m, {}, std::make_tuple(constraints...)});
}
...
internal::column_t<G, S, Op...> make_column(std::string name, S setter,
                                            G getter, Op... constraints) {
  static_assert(std::is_same<internal::setter_field_type_t<S>,
                             internal::getter_field_type_t<G>>::value,
                "Getter and setter must get and set same data type");
  static_assert(polyfill::conjunction_v<internal::is_constraint<Op>...>,
                "Incorrect constraints pack");

  SQLITE_ORM_CLANG_SUPPRESS_MISSING_BRACES(return {
      std::move(name), getter, setter, std::make_tuple(constraints...)});
}
...
template <class G, class S, class... Op,
          internal::satisfies<internal::is_getter, G> = true,
          internal::satisfies<internal::is_setter, S> = true>
internal::column_t<G, S, Op...> make_column(std::string name, G getter,
                                            S setter, Op... constraints) {
  static_assert(std::is_same<internal::setter_field_type_t<S>,
                             internal::getter_field_type_t<G>>::value,
                "Getter and setter must get and set same data type");
  static_assert(polyfill::conjunction_v<internal::is_constraint<Op>...>,
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool
    is_printable_v<T, polyfill::void_t<decltype(field_printer<T>{})>> = true
...
/**
 *  char is neither signed char nor unsigned char so it has its own
 * specialization
 */
template <>
...
struct field_printer<T,
                     std::enable_if_t<std::is_base_of<std::string, T>::value>> {
  std::string operator()(std::string string) const { return string; }
};
...
template <class T>
struct field_printer<
    T, std::enable_if_t<std::is_base_of<std::wstring, T>::value>> {
...
struct field_printer<nullptr_t, void> {
  std::string operator()(const nullptr_t&) const { return "null"; }
};
#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
template <>
struct field_printer<std::nullopt_t, void> {
  std::string operator()(const std::nullopt_t&) const { return "null"; }
};
...
using is_base_of_template =
    decltype(is_base_of_template_impl<C>::test(std::declval<T>()));
#else
...
template <typename T, template <typename...> class C>
SQLITE_ORM_INLINE_VAR constexpr bool is_base_of_template_v =
    is_base_of_template<T, C>::value;
}  // namespace internal
}  // namespace sqlite_orm
...
/** @short Base class for a custom table alias, column alias or expression
 * alias.
 */
struct alias_tag {};
...
template <class A>
SQLITE_ORM_INLINE_VAR constexpr bool is_alias_v =
    std::is_base_of<alias_tag, A>::value;
...
SQLITE_ORM_INLINE_VAR constexpr bool is_column_alias_v =
    polyfill::conjunction_v<
        is_alias<A>, polyfill::negation<polyfill::is_detected<type_t, A>>>;
...
/*
 *  Protect an otherwise bindable element so that it is always serialized as a
 * literal value.
 */
template <class T>
...
struct limit_string {
  operator std::string() const { return "LIMIT"; }
};
...
struct collate_t : public condition_t {
  ...
  operator std::string() const { return collate_constraint_t{this->argument}; }
};

struct named_collate_base {
  std::string name;

  operator std::string() const { return "COLLATE " + this->name; }
};
...
struct negated_condition_string {
  operator std::string() const { return "NOT"; }
};
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_binary_condition_v =
    is_base_of_template_v<T, binary_condition>;

template <class T>
using is_binary_condition = polyfill::bool_constant<is_binary_condition_v<T>>;

struct and_condition_string {
  operator std::string() const { return "AND"; }
};
...
struct or_condition_string {
  operator std::string() const { return "OR"; }
};
 ...
 */
template <class L, class R>
struct is_not_equal_t : binary_condition<L, R, is_not_equal_string, bool>,
                        negatable_t {
 ...
 */
template <class L, class R>
struct greater_than_t : binary_condition<L, R, greater_than_string, bool>,
                        negatable_t {
...
struct greater_or_equal_t
    : binary_condition<L, R, greater_or_equal_string, bool>,
      negatable_t {
 ...
 */
template <class L, class R>
struct lesser_than_t : binary_condition<L, R, lesser_than_string, bool>,
                       negatable_t {
 ...
 */
template <class L, class R>
struct lesser_or_equal_t : binary_condition<L, R, lesser_or_equal_string, bool>,
                           negatable_t {
...
struct is_null_string {
  operator std::string() const { return "IS NULL"; }
};
...
struct is_not_null_string {
  operator std::string() const { return "IS NOT NULL"; }
};
...
struct order_by_base {
  ...
  order_by_base(decltype(asc_desc) asc_desc_,
                decltype(_collate_argument) _collate_argument_)
      : asc_desc(asc_desc_), _collate_argument(std::move(_collate_argument_)) {}
#endif
};

struct order_by_string {
  operator std::string() const { return "ORDER BY"; }
};
...
struct order_by_t : order_by_base, order_by_string {
  ...
  self collate_binary() const {
    auto res = *this;
    res._collate_argument = collate_constraint_t::string_from_collate_argument(
        collate_argument::binary);
    return res;
  }
...
struct dynamic_order_by_entry_t : order_by_base {
  ...
  dynamic_order_by_entry_t(decltype(name) name_, int asc_desc_,
                           std::string collate_argument_)
      : order_by_base{asc_desc_, std::move(collate_argument_)},
        name(std::move(name_)) {}
};
...
struct dynamic_order_by_t : order_by_string {
  ...
  void push_back(order_by_t<O> order_by) {
    ...
    this->entries.emplace_back(std::move(columnName), order_by.asc_desc,
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_order_by_v = polyfill::disjunction_v<
    polyfill::is_specialization_of<T, order_by_t>,
...
struct between_string {
  operator std::string() const { return "BETWEEN"; }
};
...
struct like_string {
  operator std::string() const { return "LIKE"; }
};
...
struct glob_string {
  operator std::string() const { return "GLOB"; }
};
...
struct cross_join_string {
  operator std::string() const { return "CROSS JOIN"; }
};
...
struct natural_join_string {
  operator std::string() const { return "NATURAL JOIN"; }
};
...
struct left_join_string {
  operator std::string() const { return "LEFT JOIN"; }
};
...
struct left_outer_join_string {
  operator std::string() const { return "LEFT OUTER JOIN"; }
};
...
struct on_string {
  operator std::string() const { return "ON"; }
};
...
struct using_t {
  ...
  operator std::string() const { return "USING"; }
};

struct inner_join_string {
  operator std::string() const { return "INNER JOIN"; }
};
...
struct cast_string {
  operator std::string() const { return "CAST"; }
};
...
template <class T,
          internal::satisfies<std::is_base_of, internal::negatable_t, T> = true>
...
template <class T, class O,
          internal::satisfies_not<internal::is_offset, T> = true>
...
template <class L, class R,
          std::enable_if_t<polyfill::disjunction_v<
                               std::is_base_of<internal::condition_t, L>,
                               std::is_base_of<internal::condition_t, R>>,
                           bool> = true>
auto operator&&(L l, R r) {
  using internal::get_from_expression;
  return internal::make_and_condition(std::move(get_from_expression(l)),
                                      std::move(get_from_expression(r)));
}
...
auto operator||(L l, R r) {
  using internal::get_from_expression;
  return internal::make_or_condition(std::move(get_from_expression(l)),
                                     std::move(get_from_expression(r)));
}
...
template <class L, class E>
internal::dynamic_in_t<L, std::vector<E>> in(L l,
...
template <class L, class E>
internal::dynamic_in_t<L, std::vector<E>> not_in(
    L l, std::initializer_list<E> values) {
  return {std::move(l), std::move(values), true};
}
 ...
 */
template <class O, internal::satisfies_not<std::is_base_of, integer_printer,
                                           type_printer<O>> = true>
 ...
 */
template <class O, internal::satisfies<std::is_base_of, integer_printer,
                                       type_printer<O>> = true>
...
internal::dynamic_order_by_t<
    internal::serializer_context<typename S::db_objects_type>>
...
struct recordset_alias : alias_tag {
  ...
  static std::string get() { return {A, X...}; }
};
...
struct alias_extractor {
  static std::string extract() { return {}; }

  static std::string as_alias() { return {}; }
};
...
struct alias_extractor<A, match_if<is_alias, A>> {
  ...
  template <
      class T = A,
      satisfies_not<std::is_same, polyfill::detected_t<type_t, T>, A> = true>
...
/**
 *  This is a common built-in class used for custom single-character column
 * aliases. For convenience there exist type aliases `colalias_a`, `colalias_b`,
...
internal::alias_column_t<A, C> alias_column(C c) {
  ...
  static_assert(std::is_same<polyfill::detected_t<internal::table_type_of_t, C>,
                             aliased_type>::value,
...
template <class A, class E,
          internal::satisfies<internal::is_column_alias, A> = true>
...
SQLITE_ORM_INLINE_VAR constexpr bool is_built_in_function_v =
    is_base_of_template_v<T, built_in_function_t>;
...
struct built_in_aggregate_function_t : built_in_function_t<R, S, Args...> {
  ...
  template <class W>
  filtered_aggregate_function<built_in_aggregate_function_t<R, S, Args...>, W>
  filter(where_t<W> wh) {
...
struct typeof_string {
  serialize_result_type serialize() const { return "TYPEOF"; }
};

struct unicode_string {
  serialize_result_type serialize() const { return "UNICODE"; }
};

struct length_string {
  serialize_result_type serialize() const { return "LENGTH"; }
};

struct abs_string {
  serialize_result_type serialize() const { return "ABS"; }
};

struct lower_string {
  serialize_result_type serialize() const { return "LOWER"; }
};

struct upper_string {
  serialize_result_type serialize() const { return "UPPER"; }
};

struct last_insert_rowid_string {
  serialize_result_type serialize() const { return "LAST_INSERT_ROWID"; }
};

struct total_changes_string {
  serialize_result_type serialize() const { return "TOTAL_CHANGES"; }
};

struct changes_string {
  serialize_result_type serialize() const { return "CHANGES"; }
};

struct trim_string {
  serialize_result_type serialize() const { return "TRIM"; }
};

struct ltrim_string {
  serialize_result_type serialize() const { return "LTRIM"; }
};

struct rtrim_string {
  serialize_result_type serialize() const { return "RTRIM"; }
};

struct hex_string {
  serialize_result_type serialize() const { return "HEX"; }
};

struct quote_string {
  serialize_result_type serialize() const { return "QUOTE"; }
};

struct randomblob_string {
  serialize_result_type serialize() const { return "RANDOMBLOB"; }
};

struct instr_string {
  serialize_result_type serialize() const { return "INSTR"; }
};

struct replace_string {
  serialize_result_type serialize() const { return "REPLACE"; }
};

struct round_string {
  serialize_result_type serialize() const { return "ROUND"; }
};

#if SQLITE_VERSION_NUMBER >= 3007016

struct char_string {
  serialize_result_type serialize() const { return "CHAR"; }
};

struct random_string {
  serialize_result_type serialize() const { return "RANDOM"; }
};

#endif

struct coalesce_string {
  serialize_result_type serialize() const { return "COALESCE"; }
};

struct ifnull_string {
  serialize_result_type serialize() const { return "IFNULL"; }
};

struct nullif_string {
  serialize_result_type serialize() const { return "NULLIF"; }
};

struct date_string {
  serialize_result_type serialize() const { return "DATE"; }
};

struct time_string {
  serialize_result_type serialize() const { return "TIME"; }
};

struct datetime_string {
  serialize_result_type serialize() const { return "DATETIME"; }
};

struct julianday_string {
  serialize_result_type serialize() const { return "JULIANDAY"; }
};

struct strftime_string {
  serialize_result_type serialize() const { return "STRFTIME"; }
};

struct zeroblob_string {
  serialize_result_type serialize() const { return "ZEROBLOB"; }
};

struct substr_string {
  serialize_result_type serialize() const { return "SUBSTR"; }
};
#ifdef SQLITE_SOUNDEX
struct soundex_string {
  serialize_result_type serialize() const { return "SOUNDEX"; }
};
#endif
struct total_string {
  serialize_result_type serialize() const { return "TOTAL"; }
};

struct sum_string {
  serialize_result_type serialize() const { return "SUM"; }
};

struct count_string {
  serialize_result_type serialize() const { return "COUNT"; }
};
...
struct avg_string {
  serialize_result_type serialize() const { return "AVG"; }
};

struct max_string {
  serialize_result_type serialize() const { return "MAX"; }
};

struct min_string {
  serialize_result_type serialize() const { return "MIN"; }
};

struct group_concat_string {
  serialize_result_type serialize() const { return "GROUP_CONCAT"; }
};
#ifdef SQLITE_ENABLE_MATH_FUNCTIONS
struct acos_string {
  serialize_result_type serialize() const { return "ACOS"; }
};

struct acosh_string {
  serialize_result_type serialize() const { return "ACOSH"; }
};

struct asin_string {
  serialize_result_type serialize() const { return "ASIN"; }
};

struct asinh_string {
  serialize_result_type serialize() const { return "ASINH"; }
};

struct atan_string {
  serialize_result_type serialize() const { return "ATAN"; }
};

struct atan2_string {
  serialize_result_type serialize() const { return "ATAN2"; }
};

struct atanh_string {
  serialize_result_type serialize() const { return "ATANH"; }
};

struct ceil_string {
  serialize_result_type serialize() const { return "CEIL"; }
};

struct ceiling_string {
  serialize_result_type serialize() const { return "CEILING"; }
};

struct cos_string {
  serialize_result_type serialize() const { return "COS"; }
};

struct cosh_string {
  serialize_result_type serialize() const { return "COSH"; }
};

struct degrees_string {
  serialize_result_type serialize() const { return "DEGREES"; }
};

struct exp_string {
  serialize_result_type serialize() const { return "EXP"; }
};

struct floor_string {
  serialize_result_type serialize() const { return "FLOOR"; }
};

struct ln_string {
  serialize_result_type serialize() const { return "LN"; }
};

struct log_string {
  serialize_result_type serialize() const { return "LOG"; }
};

struct log10_string {
  serialize_result_type serialize() const { return "LOG10"; }
};

struct log2_string {
  serialize_result_type serialize() const { return "LOG2"; }
};

struct mod_string {
  serialize_result_type serialize() const { return "MOD"; }
};

struct pi_string {
  serialize_result_type serialize() const { return "PI"; }
};

struct pow_string {
  serialize_result_type serialize() const { return "POW"; }
};

struct power_string {
  serialize_result_type serialize() const { return "POWER"; }
};

struct radians_string {
  serialize_result_type serialize() const { return "RADIANS"; }
};

struct sin_string {
  serialize_result_type serialize() const { return "SIN"; }
};

struct sinh_string {
  serialize_result_type serialize() const { return "SINH"; }
};

struct sqrt_string {
  serialize_result_type serialize() const { return "SQRT"; }
};

struct tan_string {
  serialize_result_type serialize() const { return "TAN"; }
};

struct tanh_string {
  serialize_result_type serialize() const { return "TANH"; }
};

struct trunc_string {
  serialize_result_type serialize() const { return "TRUNC"; }
};

#endif  //  SQLITE_ENABLE_MATH_FUNCTIONS
#ifdef SQLITE_ENABLE_JSON1
struct json_string {
  serialize_result_type serialize() const { return "JSON"; }
};

struct json_array_string {
  serialize_result_type serialize() const { return "JSON_ARRAY"; }
};

struct json_array_length_string {
  serialize_result_type serialize() const { return "JSON_ARRAY_LENGTH"; }
};

struct json_extract_string {
  serialize_result_type serialize() const { return "JSON_EXTRACT"; }
};

struct json_insert_string {
  serialize_result_type serialize() const { return "JSON_INSERT"; }
};

struct json_replace_string {
  serialize_result_type serialize() const { return "JSON_REPLACE"; }
};

struct json_set_string {
  serialize_result_type serialize() const { return "JSON_SET"; }
};

struct json_object_string {
  serialize_result_type serialize() const { return "JSON_OBJECT"; }
};

struct json_patch_string {
  serialize_result_type serialize() const { return "JSON_PATCH"; }
};

struct json_remove_string {
  serialize_result_type serialize() const { return "JSON_REMOVE"; }
};

struct json_type_string {
  serialize_result_type serialize() const { return "JSON_TYPE"; }
};

struct json_valid_string {
  serialize_result_type serialize() const { return "JSON_VALID"; }
};

struct json_quote_string {
  serialize_result_type serialize() const { return "JSON_QUOTE"; }
};

struct json_group_array_string {
  serialize_result_type serialize() const { return "JSON_GROUP_ARRAY"; }
};

struct json_group_object_string {
  serialize_result_type serialize() const { return "JSON_GROUP_OBJECT"; }
};
#endif  //  SQLITE_ENABLE_JSON1

template <class T>
using field_type_or_type_t =
    polyfill::detected_or_t<T, type_t, member_field_type<T>>;
}  // namespace internal
...
template <class F, class R,
          internal::satisfies<internal::is_built_in_function, F> = true>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::acos(&Triangle::cornerA));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *  ACOS(X) function https://www.sqlite.org/lang_mathfunc.html#acos
 *
 *  Difference with the previous function is that previous override has `double`
 * as return type but this override accepts return type from you as a template
 * argument. You can use any bindable type: `float`, `int`,
 * `std::optional<double>` etc. This override is handy when you expect `null` as
 * result.
 *
 *  Example:
 *
 *  auto rows =
 * storage.select(sqlite_orm::acos<std::optional<double>>(&Triangle::cornerA));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::acosh(&Triangle::cornerA));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::acosh<std::optional<double>>(&Triangle::cornerA));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::asin(&Triangle::cornerA));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::asin<std::optional<double>>(&Triangle::cornerA));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::asinh(&Triangle::cornerA));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::asinh<std::optional<double>>(&Triangle::cornerA));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::atan(1));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::atan<std::optional<double>>(1));   //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::atan2(1, 3));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X, class Y>
internal::built_in_function_t<double, internal::atan2_string, X, Y> atan2(X x,
                                                                          Y y) {
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::atan2<std::optional<double>>(1, 3));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::atanh(1));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::atanh<std::optional<double>>(1)); //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::ceil(&User::rating));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::ceil<std::optional<double>>(&User::rating));   //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::ceiling(&User::rating));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
internal::built_in_function_t<double, internal::ceiling_string, X> ceiling(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::ceiling<std::optional<double>>(&User::rating)); //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::cos(&Triangle::cornerB));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::cos<std::optional<double>>(&User::rating));   //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::cosh(&Triangle::cornerB));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::cosh<std::optional<double>>(&User::rating));   //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::degrees(&Triangle::cornerB));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
internal::built_in_function_t<double, internal::degrees_string, X> degrees(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::degrees<std::optional<double>>(&User::rating)); //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::exp(&Triangle::cornerB));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::exp<std::optional<double>>(&User::rating));   //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::floor(&User::rating));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::floor<std::optional<double>>(&User::rating));   //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::ln(200));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::ln<std::optional<double>>(200));   //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::log(100));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::log<std::optional<double>>(100)); //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::log10(100));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::log10<std::optional<double>>(100));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::log(10, 100));   //  decltype(rows)
 * is std::vector<double>
 */
template <class B, class X>
internal::built_in_function_t<double, internal::log_string, B, X> log(B b,
                                                                      X x) {
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::log<std::optional<double>>(10, 100));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::log2(64));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::log2<std::optional<double>>(64)); //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::mod_f(6, 5));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X, class Y>
internal::built_in_function_t<double, internal::mod_string, X, Y> mod_f(X x,
                                                                        Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::pi());   //  decltype(rows) is
 * std::vector<double>
 */
 ...
 *  Difference with the previous function is that previous override has `double`
 * as return type but this override accepts return type from you as a template
 * argument. You can use any bindable type: `float`, `int`, etc.
 *
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::pi<float>());   //  decltype(rows) is
 * std::vector<float>
 */
template <class R>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::pow(2, 5));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X, class Y>
internal::built_in_function_t<double, internal::pow_string, X, Y> pow(X x,
                                                                      Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::pow<std::optional<double>>(2, 5)); //
 * decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X, class Y>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::power(2, 5));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X, class Y>
internal::built_in_function_t<double, internal::power_string, X, Y> power(X x,
                                                                          Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::power<std::optional<double>>(2, 5));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X, class Y>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::radians(&Triangle::cornerAInDegrees));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
internal::built_in_function_t<double, internal::radians_string, X> radians(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::radians<std::optional<double>>(&Triangle::cornerAInDegrees));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::sin(&Triangle::cornerA));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::sin<std::optional<double>>(&Triangle::cornerA));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::sinh(&Triangle::cornerA));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *
 *  auto rows =
 * storage.select(sqlite_orm::sinh<std::optional<double>>(&Triangle::cornerA));
 * //  decltype(rows) is std::vector<std::optional<double>>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::sqrt(25));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::sqrt<int>(25));   //  decltype(rows)
 * is std::vector<int>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::tan(&Triangle::cornerC));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::tan<float>(&Triangle::cornerC));   //
 * decltype(rows) is std::vector<float>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::tanh(&Triangle::cornerC));   //
 * decltype(rows) is std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::tanh<float>(&Triangle::cornerC)); //
 * decltype(rows) is std::vector<float>
 */
template <class R, class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::trunc(5.5));   //  decltype(rows) is
 * std::vector<double>
 */
template <class X>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::trunc<float>(5.5));   //
 * decltype(rows) is std::vector<float>
 */
template <class R, class X>
 ...
 */
template <class T>
internal::built_in_function_t<std::string, internal::typeof_string, T> typeof_(
    T t) {
 ...
 */
template <class T>
internal::built_in_function_t<std::unique_ptr<double>, internal::abs_string, T>
abs(T t) {
  return {std::tuple<T>{std::forward<T>(t)}};
}
 ...
 */
template <class T>
internal::built_in_function_t<std::string, internal::lower_string, T> lower(
    T t) {
  return {std::tuple<T>{std::forward<T>(t)}};
}
 ...
 */
template <class T>
internal::built_in_function_t<std::string, internal::upper_string, T> upper(
    T t) {
  return {std::tuple<T>{std::forward<T>(t)}};
}

/**
 *  LAST_INSERT_ROWID(x) function
 * https://www.sqlite.org/lang_corefunc.html#last_insert_rowid
 */
inline internal::built_in_function_t<int64, internal::last_insert_rowid_string>
last_insert_rowid() {
  return {{}};
}
 ...
 */
inline internal::built_in_function_t<int, internal::total_changes_string>
total_changes() {
  return {{}};
}
 ...
 */
template <class X, class Y>
internal::built_in_function_t<std::string, internal::trim_string, X, Y> trim(
    X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
 ...
 */
template <class X>
internal::built_in_function_t<std::string, internal::ltrim_string, X> ltrim(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 */
template <class X, class Y>
internal::built_in_function_t<std::string, internal::ltrim_string, X, Y> ltrim(
    X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
 ...
 */
template <class X>
internal::built_in_function_t<std::string, internal::rtrim_string, X> rtrim(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 */
template <class X, class Y>
internal::built_in_function_t<std::string, internal::rtrim_string, X, Y> rtrim(
    X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
 ...
 */
template <class X>
internal::built_in_function_t<std::string, internal::quote_string, X> quote(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 */
template <class X>
internal::built_in_function_t<std::vector<char>, internal::randomblob_string, X>
randomblob(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 */
template <class X, class Y>
internal::built_in_function_t<int, internal::instr_string, X, Y> instr(X x,
                                                                       Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
...
template <class X, class Y, class Z,
          std::enable_if_t<internal::count_tuple<std::tuple<X, Y, Z>,
                                                 internal::is_into>::value == 0,
                           bool> = true>
internal::built_in_function_t<std::string, internal::replace_string, X, Y, Z>
replace(X x, Y y, Z z) {
  return {std::tuple<X, Y, Z>{std::forward<X>(x), std::forward<Y>(y),
                              std::forward<Z>(z)}};
}
 ...
 */
template <class X, class Y>
internal::built_in_function_t<double, internal::round_string, X, Y> round(X x,
                                                                          Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
 ...
 */
template <class... Args>
internal::built_in_function_t<std::string, internal::char_string, Args...>
char_(Args... args) {
  return {std::make_tuple(std::forward<Args>(args)...)};
}
...
auto ifnull(X x, Y y) -> internal::built_in_function_t<
    typename std::conditional_t<  //  choose R or common type
        std::is_void<R>::value,
        std::common_type<internal::field_type_or_type_t<X>,
                         internal::field_type_or_type_t<Y>>,
        polyfill::type_identity<R>>::type,
    internal::ifnull_string, X, Y> {
 ...
 */
    ...
    defined(SQLITE_ORM_IF_CONSTEXPR_SUPPORTED)
/**
...
template <class R = void, class X, class Y,
          std::enable_if_t<
              polyfill::disjunction_v<
                  polyfill::negation<std::is_void<R>>,
...
auto nullif(X x, Y y) {
  if constexpr (std::is_void_v<R>) {
    using F = internal::built_in_function_t<
        std::optional<std::common_type_t<internal::field_type_or_type_t<X>,
                                         internal::field_type_or_type_t<Y>>>,
        internal::nullif_string, X, Y>;
...
#else
template <class R, class X, class Y>
internal::built_in_function_t<R, internal::nullif_string, X, Y> nullif(X x,
                                                                       Y y) {
  return {std::make_tuple(std::move(x), std::move(y))};
}
#endif
 ...
 */
template <class... Args>
internal::built_in_function_t<std::string, internal::date_string, Args...> date(
    Args... args) {
 ...
 */
template <class... Args>
internal::built_in_function_t<std::string, internal::time_string, Args...> time(
    Args... args) {
  return {std::tuple<Args...>{std::forward<Args>(args)...}};
}
 ...
 */
template <class... Args>
internal::built_in_function_t<std::string, internal::datetime_string, Args...>
datetime(Args... args) {
  return {std::tuple<Args...>{std::forward<Args>(args)...}};
}
 ...
 */
template <class... Args>
internal::built_in_function_t<double, internal::julianday_string, Args...>
julianday(Args... args) {
  return {std::tuple<Args...>{std::forward<Args>(args)...}};
}
 ...
 */
template <class... Args>
internal::built_in_function_t<std::string, internal::strftime_string, Args...>
strftime(Args... args) {
  return {std::tuple<Args...>{std::forward<Args>(args)...}};
}
 ...
 */
template <class N>
internal::built_in_function_t<std::vector<char>, internal::zeroblob_string, N>
zeroblob(N n) {
 ...
 */
template <class X, class Y>
internal::built_in_function_t<std::string, internal::substr_string, X, Y>
substr(X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}
 ...
 */
template <class X, class Y, class Z>
internal::built_in_function_t<std::string, internal::substr_string, X, Y, Z>
substr(X x, Y y, Z z) {
  return {std::tuple<X, Y, Z>{std::forward<X>(x), std::forward<Y>(y),
                              std::forward<Z>(z)}};
}
 ...
 */
template <class X>
internal::built_in_function_t<std::string, internal::soundex_string, X> soundex(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
#endif
 ...
 */
template <class X>
internal::built_in_aggregate_function_t<double, internal::total_string, X>
total(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 */
template <class X>
internal::built_in_aggregate_function_t<std::unique_ptr<double>,
                                        internal::sum_string, X>
sum(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 */
template <class X>
internal::built_in_aggregate_function_t<int, internal::count_string, X> count(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 */
inline internal::count_asterisk_without_type count() { return {}; }
 ...
 */
template <class X>
internal::built_in_aggregate_function_t<double, internal::avg_string, X> avg(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
 ...
 */
template <class X>
internal::built_in_aggregate_function_t<internal::unique_ptr_result_of<X>,
                                        internal::max_string, X>
max(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
...
internal::built_in_function_t<internal::unique_ptr_result_of<X>,
                              internal::max_string, X, Y, Rest...>
max(X x, Y y, Rest... rest) {
  return {std::tuple<X, Y, Rest...>{std::forward<X>(x), std::forward<Y>(y),
 ...
 */
template <class X>
internal::built_in_aggregate_function_t<std::string,
                                        internal::group_concat_string, X>
group_concat(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
...
template <class X, class Y>
internal::built_in_aggregate_function_t<std::string,
                                        internal::group_concat_string, X, Y>
...
template <class... Args>
internal::built_in_function_t<std::string, internal::json_array_string, Args...>
json_array(Args... args) {
  return {std::tuple<Args...>{std::forward<Args>(args)...}};
}

template <class X>
internal::built_in_function_t<int, internal::json_array_length_string, X>
json_array_length(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}

template <class R, class X>
internal::built_in_function_t<R, internal::json_array_length_string, X>
json_array_length(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}

template <class X, class Y>
internal::built_in_function_t<int, internal::json_array_length_string, X, Y>
json_array_length(X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}

template <class R, class X, class Y>
internal::built_in_function_t<R, internal::json_array_length_string, X, Y>
json_array_length(X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}

template <class R, class X, class... Args>
internal::built_in_function_t<R, internal::json_extract_string, X, Args...>
json_extract(X x, Args... args) {
  return {
      std::tuple<X, Args...>{std::forward<X>(x), std::forward<Args>(args)...}};
}

template <class X, class... Args>
internal::built_in_function_t<std::string, internal::json_insert_string, X,
                              Args...>
...
template <class X, class... Args>
internal::built_in_function_t<std::string, internal::json_replace_string, X,
                              Args...>
...
template <class X, class... Args>
internal::built_in_function_t<std::string, internal::json_set_string, X,
                              Args...>
json_set(X x, Args... args) {
  static_assert(std::tuple_size<std::tuple<Args...>>::value % 2 == 0,
...
template <class... Args>
internal::built_in_function_t<std::string, internal::json_object_string,
                              Args...>
...
template <class X, class Y>
internal::built_in_function_t<std::string, internal::json_patch_string, X, Y>
json_patch(X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}

template <class X, class... Args>
internal::built_in_function_t<std::string, internal::json_remove_string, X,
                              Args...>
...
template <class R, class X, class... Args>
internal::built_in_function_t<R, internal::json_remove_string, X, Args...>
json_remove(X x, Args... args) {
  return {
      std::tuple<X, Args...>{std::forward<X>(x), std::forward<Args>(args)...}};
}
...
template <class X>
internal::built_in_function_t<std::string, internal::json_type_string, X>
json_type(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}
...
template <class X, class Y>
internal::built_in_function_t<std::string, internal::json_type_string, X, Y>
json_type(X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}

template <class R, class X, class Y>
internal::built_in_function_t<R, internal::json_type_string, X, Y> json_type(
    X x, Y y) {
  return {std::tuple<X, Y>{std::forward<X>(x), std::forward<Y>(y)}};
}

template <class X>
internal::built_in_function_t<bool, internal::json_valid_string, X> json_valid(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}

template <class R, class X>
internal::built_in_function_t<R, internal::json_quote_string, X> json_quote(
    X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}

template <class X>
internal::built_in_function_t<std::string, internal::json_group_array_string, X>
json_group_array(X x) {
  return {std::tuple<X>{std::forward<X>(x)}};
}

template <class X, class Y>
internal::built_in_function_t<std::string, internal::json_group_object_string,
                              X, Y>
...
template <class L, class R,
          std::enable_if_t<polyfill::disjunction_v<
                               std::is_base_of<internal::arithmetic_t, L>,
...
struct where_string {
  serialize_result_type serialize() const { return "WHERE"; }
};

/**
 *  WHERE argument holder.
 *  C is expression type. Can be any expression like: is_equal_t, is_null_t,
 * exists_t etc Don't construct it manually. Call `where(...)` function instead.
 */
template <class C>
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_where_v =
    polyfill::is_specialization_of_v<T, where_t>;
 ...
 *  //  FROM letters
 *  //  WHERE id > 3
 *  auto rows = storage.select(&Letter::name, where(greater_than(&Letter::id,
 * 3)));
 */
template <class C>
...
template <class T>
using is_group_by = polyfill::disjunction<
    polyfill::is_specialization_of<T, group_by_t>,
...
/**
 *  [Deprecation notice]: this function is deprecated and will be removed in
 * v1.9. Please use `group_by(...).having(...)` instead.
 *
 *  HAVING(expression).
 *  Example: storage.get_all<Employee>(group_by(&Employee::name),
 ...
 */
template <class T>
[[deprecated("Use group_by(...).having(...) instead")]] internal::having_t<T>
having(T expression) {
  return {std::move(expression)};
}
}  // namespace sqlite_orm
...
SQLITE_ORM_INLINE_VAR constexpr bool is_column_pointer_v =
    polyfill::is_specialization_of_v<T, column_pointer>;
...
struct distinct_string {
  operator std::string() const { return "DISTINCT"; }
};
...
struct all_string {
  operator std::string() const { return "ALL"; }
};
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_columns_v =
    polyfill::is_specialization_of_v<T, columns_t>;
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_select_v =
    polyfill::is_specialization_of_v<T, select_t>;
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_compound_operator_v =
    is_base_of_template_v<T, compound_operator>;
...
struct except_string {
  operator std::string() const { return "EXCEPT"; }
};
...
struct intersect_string {
  operator std::string() const { return "INTERSECT"; }
};
/**
...
struct simple_case_builder {
  ...
  simple_case_builder<R, T, E, Args..., std::pair<W, Th>> when(W w,
                                                               then_t<Th> t) {
    ...
    result_args_type result_args =
        std::tuple_cat(std::move(this->args), std::make_tuple(newPair));
    std::get<std::tuple_size<result_args_type>::value - 1>(result_args) =
        std::move(newPair);
    return {std::move(this->case_expression), std::move(result_args),
            std::move(this->else_expression)};
  }

  simple_case_t<R, T, E, Args...> end() {
    return {std::move(this->case_expression), std::move(args),
            std::move(this->else_expression)};
  }
...
void validate_conditions() {
  static_assert(count_tuple<T, is_where>::value <= 1,
                "a single query cannot contain > 1 WHERE blocks");
  static_assert(count_tuple<T, is_group_by>::value <= 1,
                "a single query cannot contain > 1 GROUP BY blocks");
  static_assert(count_tuple<T, is_order_by>::value <= 1,
                "a single query cannot contain > 1 ORDER BY blocks");
  static_assert(count_tuple<T, is_limit>::value <= 1,
                "a single query cannot contain > 1 LIMIT blocks");
  static_assert(count_tuple<T, is_from>::value <= 1,
 ...
 *   The default is the implicit order as returned by SQLite, which may differ
 * from the defined order if the schema of a table has been changed. By
 * specifying the defined order, the columns are written out in the resulting
 * select SQL string.
 *
 *   In pseudo code:
 ...
 *   // decltype(rows) is std::vector<std::tuple<...all columns in implicitly
 * stored order...>> Example: auto rows = storage.select(asterisk<User>(true));
 *   // decltype(rows) is std::vector<std::tuple<...all columns in declared
 * make_table order...>>
 *
 *   If you need to fetch results as objects instead of tuples please use
 * `object<T>()`.
 */
template <class T>
 ...
 *   // decltype(rows) is std::vector<User>, where the User objects are
 * constructed from columns in implicitly stored order Example: auto rows =
 * storage.select(object<User>(true));
 *   // decltype(rows) is std::vector<User>, where the User objects are
 * constructed from columns in declared make_table order
 *
 *   If you need to fetch results as tuples instead of objects please use
 * `asterisk<T>()`.
 */
template <class T>
...
#if !defined(SQLITE_ORM_AGGREGATE_NSDMI_SUPPORTED) || \
    !defined(SQLITE_ORM_AGGREGATE_PAREN_INIT_SUPPORTED)
  table_info(decltype(cid) cid_, decltype(name) name_, decltype(type) type_,
             decltype(notnull) notnull_, decltype(dflt_value) dflt_value_,
             decltype(pk) pk_)
      : cid(cid_),
        name(std::move(name_)),
        type(std::move(type_)),
        notnull(notnull_),
        dflt_value(std::move(dflt_value_)),
        pk(pk_) {}
#endif
};
...
#if !defined(SQLITE_ORM_AGGREGATE_NSDMI_SUPPORTED) || \
    !defined(SQLITE_ORM_AGGREGATE_PAREN_INIT_SUPPORTED)
  table_xinfo(decltype(cid) cid_, decltype(name) name_, decltype(type) type_,
              decltype(notnull) notnull_, decltype(dflt_value) dflt_value_,
              decltype(pk) pk_, decltype(hidden) hidden_)
      : cid(cid_),
        name(std::move(name_)),
        type(std::move(type_)),
        notnull(notnull_),
...
// NOTE Idea : Maybe also implement a custom trigger system to call a c++
// callback when a trigger triggers ? (Could be implemented with a normal
// trigger that insert or update an internal table and then retreive the event
// in the C++ code, to call the C++ user callback, with update hooks:
// https://www.sqlite.org/c3ref/update_hook.html) It could be an interesting
// feature to bring to sqlite_orm that other libraries don't have ?
...
struct partial_trigger_t {
  ...
  partial_trigger_t(T trigger_base, S... statements)
      : base{std::move(trigger_base)},
        statements{std::make_tuple<S...>(std::forward<S>(statements)...)} {}

  partial_trigger_t& end() { return *this; }
};
...
struct trigger_t : base_trigger {
  ...
  trigger_t(std::string name, T trigger_base, elements_type statements)
      : base_trigger{std::move(name)},
        base(std::move(trigger_base)),
        elements(std::move(statements)) {}
#endif
};
...
struct trigger_base_t {
  ...
  /**
   * Value used to determine if we execute the trigger on each row or on each
   * statement (SQLite doesn't support the FOR EACH STATEMENT syntax yet:
   * https://sqlite.org/lang_createtrigger.html#description so this value is
   * more of a placeholder for a later update)
   */
...
struct trigger_update_type_t : trigger_type_base_t {
  ...
  trigger_update_type_t(trigger_timing timing, trigger_type type, Cs... columns)
      : trigger_type_base_t(timing, type),
        columns(std::make_tuple<Cs...>(std::forward<Cs>(columns)...)) {}
...
template <class T, class... S>
internal::trigger_t<T, S...> make_trigger(
    std::string name, const internal::partial_trigger_t<T, S...>& part) {
  SQLITE_ORM_CLANG_SUPPRESS_MISSING_BRACES(return {
      std::move(name), std::move(part.base), std::move(part.statements)});
}
 ...
 */
using statement_finalizer = std::unique_ptr<
    sqlite3_stmt,
    std::integral_constant<decltype(&sqlite3_finalize), sqlite3_finalize>>;
}  // namespace sqlite_orm
#pragma once
#include <type_traits>
...
using arithmetic_tag_t =
    std::conditional_t<std::is_integral<V>::value,
                       // Integer class
                       std::conditional_t<sizeof(V) <= sizeof(int),
                                          int_or_smaller_tag, bigint_tag>,
...
concept stateless_deleter =
    std::is_empty_v<D> && std::is_default_constructible_v<D>;
...
struct is_integral_fp_c<
    D, polyfill::void_t<typename D::value_type, decltype(D::value),
                        std::enable_if_t<std::is_function<std::remove_pointer_t<
                            typename D::value_type>>::value>>>
    : std::true_type {};
template <typename D>
SQLITE_ORM_INLINE_VAR constexpr bool is_integral_fp_c_v =
    is_integral_fp_c<D>::value;

template <typename D, typename SFINAE = void>
struct can_yield_fp : std::false_type {};
template <typename D>
struct can_yield_fp<
    D, polyfill::void_t<decltype(+std::declval<D>()),
                        std::enable_if_t<std::is_function<std::remove_pointer_t<
                            decltype(+std::declval<D>())>>::value>>>
    : std::true_type {};
template <typename D>
...
/**
 *  This concept tests whether a deleter yields a function pointer, which is
 * convertible to an xdestroy function pointer. Note: We are using 'is
 * convertible' rather than 'is same' because of any exception specification.
 */
template <typename D>
concept yields_xdestroy =
    yields_fp<D> && std::convertible_to<yielded_fn_t<D>, xdestroy_fn_t>;
 ...
 *
 *  Requires that the deleter can be called with the q-qualified pointer
 * argument; it doesn't check so explicitly, but a compiler error will occur.
 */
template <typename D, typename P>
 ...
 *
 *  Performs a const-cast of the argument pointer in order to allow for C API
 * functions that take a non-const parameter, but user code passes a pointer to
 * a const object.
 */
...
SQLITE_ORM_INLINE_VAR constexpr bool is_unusable_for_xdestroy_v =
    ...
    (can_yield_fp_v<D> &&
     !std::is_convertible<yielded_fn_t<D>, xdestroy_fn_t>::value);
...
template <typename D, typename P,
          std::enable_if_t<!is_integral_fp_c_v<D>, bool> = true>
void xdestroy_proxy(void* p) noexcept {
  // C-casting `void* -> P*` like statement_binder<pointer_binding<P, T, D>>
  auto o = (P*)p;
  // ignoring return code
...
#if __cpp_lib_concepts >= 201907L
/**
 *  Prohibits using a yielded function pointer, which is not of type
 * xdestroy_fn_t.
 *
 ...
 *
 *  Type-safety is garanteed by checking whether the deleter or yielded function
 * pointer is invocable with the non-q-qualified pointer value.
 */
template <typename D, typename P>
constexpr xdestroy_fn_t obtain_xdestroy_for(D, P*) noexcept
...
template <
    ...
    std::enable_if_t<internal::is_unusable_for_xdestroy_v<D>, bool> = true>
...
template <typename D, typename P,
          std::enable_if_t<internal::needs_xdestroy_proxy_v<D, P>, bool> = true>
constexpr xdestroy_fn_t obtain_xdestroy_for(D, P*) noexcept {
  return internal::xdestroy_proxy<D, P>;
}

template <typename D, typename P,
          std::enable_if_t<internal::can_yield_xdestroy_v<D>, bool> = true>
constexpr xdestroy_fn_t obtain_xdestroy_for(D d, P*) noexcept {
  return d;
}
#endif
 ...
 *    - T: An integral constant string denoting the pointer type, e.g.
 * `carray_pvt_name`.
 *
 */
...
struct pointer_arg {
  static_assert(
      std::is_convertible<typename T::value_type, const char*>::value,
  ...
  operator P*() const noexcept { return p_; }
};
 ...
 *  ```
 *  int64 rememberedId;
 *  storage.select(func<remember_fn>(&Object::id,
 * statically_bindable_carray_pointer(&rememberedId)));
 *  ```
 */
...
class pointer_binding {
 ...
 protected:
  ...
  template <class B>
  friend B bindable_pointer(typename B::qualified_type*,
                            typename B::deleter_type) noexcept;
 ...
 public:
  ...
  ~pointer_binding() {
    if (p_) {
      if (auto xDestroy = get_xdestroy()) {
        // note: C-casting `P* -> void*` like
        // statement_binder<pointer_binding<P, T, D>>
        xDestroy((void*)p_);
      }
    }
  }
  ...
  P* take_ptr() noexcept { return std::exchange(p_, nullptr); }
...
/**
 *  Wrap a pointer, its type and its deleter function for binding it to a
 * statement.
 *
 *  Unless the deleter yields a nullptr 'xDestroy' function the ownership of the
 * pointed-to-object is transferred to the pointer binding, which will delete it
 * through the deleter when the statement finishes.
 */
...
auto bindable_pointer(std::unique_ptr<P, D> p) noexcept
    -> pointer_binding<P, T, D> {
  return bindable_pointer<T>(p.release(), p.get_deleter());
}

template <typename B>
B bindable_pointer(typename B::qualified_type* p,
 ...
 *
 *  Note: 'Static' means that ownership of the pointed-to-object won't be
 * transferred and sqlite assumes the object pointed to is valid throughout the
 * lifetime of a statement.
 */
template <class T, class P>
auto statically_bindable_pointer(P* p) noexcept
    -> static_pointer_binding<P, T> {
  return bindable_pointer<T>(p, null_xdestroy_f);
}

template <typename B>
B statically_bindable_pointer(
...
auto rebind_statically(const pointer_arg<P, T>& pv) noexcept
    -> static_pointer_binding<P, T> {
...
// Stupidly, clang says C++20, but `std::identity` was only implemented in
// libc++ 13 and libstd++-v3 10 (the latter is used on Linux). gcc got it right
// and reports C++20 only starting with v10. The check here doesn't care and
// checks the library versions in use.
//
// Another way of detection would be the constrained algorithms feature macro
// __cpp_lib_ranges
#if (__cplusplus >= 202002L) &&                     \
    ((!_LIBCPP_VERSION || _LIBCPP_VERSION >= 13) && \
     (!_GLIBCXX_RELEASE || _GLIBCXX_RELEASE >= 10))
using std::identity;
#else
struct identity {
  template <class T>
...
template <class Callable, class Object, class... Args,
          class Unqualified = remove_cvref_t<Callable>,
          std::enable_if_t<std::is_member_object_pointer<Unqualified>::value,
                           bool> = true>
...
template <class Callable, class Object, class... Args,
          class Unqualified = remove_cvref_t<Callable>,
          std::enable_if_t<std::is_member_function_pointer<Unqualified>::value,
                           bool> = true>
decltype(auto) invoke(Callable&& callable, Object&& object, Args&&... args) {
...
template <class Callable, class Object, class... Args,
          std::enable_if_t<
              polyfill::negation_v<polyfill::is_specialization_of<
                  member_object_type_t<std::remove_reference_t<Callable>>,
                  std::reference_wrapper>>,
              bool> = true>
decltype(auto) invoke(Callable&& callable,
                      std::reference_wrapper<Object> wrapper, Args&&... args) {
  return invoke(std::forward<Callable>(callable), wrapper.get(),
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool
    is_bindable_v<T, polyfill::void_t<decltype(statement_binder<T>())>> = true
    // note : msvc 14.0 needs the parentheses constructor, otherwise
    // `is_bindable<const char*>` isn't recognised. The strangest thing is that
    // this is mutually exclusive with `is_printable_v`.
    ;
...
struct statement_binder<pointer_binding<P, T, D>, void> {
  ...
  // ownership of pointed-to-object is left untouched and remains at prepared
  // statement's AST expression
  int bind(sqlite3_stmt* stmt, int index, const V& value) const {
    // note: C-casting `P* -> void*`, internal::xdestroy_proxy() does the
    // inverse
    return sqlite3_bind_pointer(stmt, index, (void*)value.ptr(), T::value,
                                null_xdestroy_f);
  }
  ...
  void result(sqlite3_context* context, V& value) const {
    ...
    // row_extractor<pointer_arg<P, T>>::extract() and
    // internal::xdestroy_proxy() do the inverse
    sqlite3_result_pointer(context, (void*)value.take_ptr(), T::value,
                           value.get_xdestroy());
  }
};
...
struct statement_binder<V, std::enable_if_t<std::is_arithmetic<V>::value>> {
 ...
 private:
  ...
  int bind(sqlite3_stmt* stmt, int index, const V& value,
           int_or_smaller_tag) const {
    return sqlite3_bind_int(stmt, index, static_cast<int>(value));
  }

  void result(sqlite3_context* context, const V& value,
              int_or_smaller_tag) const {
...
template <class V>
struct statement_binder<
    V, std::enable_if_t<polyfill::disjunction_v<
           std::is_base_of<std::string, V>, std::is_same<V, const char*>
#ifdef SQLITE_ORM_STRING_VIEW_SUPPORTED
           ,
  ...
  int bind(sqlite3_stmt* stmt, int index, const V& value) const {
    ...
    return sqlite3_bind_text(stmt, index, stringData.first, stringData.second,
                             SQLITE_TRANSIENT);
  }
  ...
  void result(sqlite3_context* context, const V& value) const {
    ...
    sqlite3_result_text(context, dataCopy, stringData.second,
...
struct statement_binder<
    V, std::enable_if_t<polyfill::disjunction_v<
           std::is_base_of<std::wstring, V>, std::is_same<V, const wchar_t*>
#ifdef SQLITE_ORM_STRING_VIEW_SUPPORTED
           ,
  ...
  int bind(sqlite3_stmt* stmt, int index, const V& value) const {
    auto stringData = this->string_data(value);
    std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
    std::string utf8Str = converter.to_bytes(
        stringData.first, stringData.first + stringData.second);
    return statement_binder<decltype(utf8Str)>().bind(stmt, index, utf8Str);
  }

  void result(sqlite3_context* context, const V& value) const {
    auto stringData = this->string_data(value);
    sqlite3_result_text16(context, stringData.first, stringData.second,
                          nullptr);
  }
...
template <class V>
struct statement_binder<
    V, std::enable_if_t<is_std_ptr<V>::value &&
                        internal::is_bindable_v<
                            std::remove_cv_t<typename V::element_type>>>> {
...
struct statement_binder<std::vector<char>, void> {
  int bind(sqlite3_stmt* stmt, int index,
    ...
    if (!value.empty()) {
      return sqlite3_bind_blob(stmt, index, (const void*)&value.front(),
  ...
  void result(sqlite3_context* context, const std::vector<char>& value) const {
    if (!value.empty()) {
      sqlite3_result_blob(context, (const void*)&value.front(),
...
struct statement_binder<
    V, std::enable_if_t<
           polyfill::is_specialization_of_v<V, std::optional> &&
...
struct tuple_value_binder {
  ...
  void operator()(const Tpl& tpl, Projection project) const {
    (*this)(tpl, std::make_index_sequence<std::tuple_size<Tpl>::value>{},
 ...
 private:
  ...
  void operator()(const Tpl& tpl, std::index_sequence<Idx...>,
  ...
  void operator()(const Tpl& tpl, std::index_sequence<I, Idx...>,
                  Projection project) const {
    this->bind(polyfill::invoke(project, std::get<I>(tpl)), I);
    (*this)(tpl, std::index_sequence<Idx...>{},
            std::forward<Projection>(project));
  }
...
enum class journal_mode : signed char {
  DELETE = 0,
  // An alternate enumeration value when using the Windows SDK that defines
  // DELETE as a macro.
  DELETE_ = DELETE,
  TRUNCATE = 1,
  PERSIST = 2,
...
inline std::unique_ptr<journal_mode> journal_mode_from_string(
 ...
 *
 *  @note The 'pointer-passing' interface doesn't support (and in fact
 * prohibits) extracting pointers from columns.
 */
template <class P, class T>
...
struct row_extractor<V, std::enable_if_t<std::is_arithmetic<V>::value>> {
  ...
  V extract(sqlite3_value* value) const { return this->extract(value, tag()); }
 ...
 private:
  ...
  V extract(sqlite3_stmt* stmt, int columnIndex,
            const int_or_smaller_tag&) const {
...
struct row_extractor<V, std::enable_if_t<is_std_ptr<V>::value>> {
  ...
  V extract(sqlite3_stmt* stmt, int columnIndex) const {
    ...
    if (type != SQLITE_NULL) {
      return is_std_ptr<V>::make(
          row_extractor<unqualified_type>().extract(stmt, columnIndex));
    } else {
      return {};
    }
  }
  ...
  V extract(sqlite3_value* value) const {
    ...
    if (type != SQLITE_NULL) {
      return is_std_ptr<V>::make(
          row_extractor<unqualified_type>().extract(value));
    } else {
      return {};
    }
  }
...
template <class V>
struct row_extractor<
    V, std::enable_if_t<polyfill::is_specialization_of_v<V, std::optional>>> {
  using unqualified_type = std::remove_cv_t<typename V::value_type>;
...
struct row_extractor<nullptr_t> {
  nullptr_t extract(const char* /*row_value*/) const { return nullptr; }

  nullptr_t extract(sqlite3_stmt*, int /*columnIndex*/) const {
    return nullptr;
  }

  nullptr_t extract(sqlite3_value*) const { return nullptr; }
};
/**
...
struct row_extractor<std::tuple<Args...>> {
 ...
 protected:
  template <size_t... Idx>
  std::tuple<Args...> extract(sqlite3_stmt* stmt,
...
inline std::string sql_escape(std::string str, char char2Escape) {
  for (size_t pos = 0; (pos = str.find(char2Escape, pos)) != str.npos;
       pos += 2) {
...
inline void perform_exec(sqlite3* db, const char* query,
...
inline void perform_exec(sqlite3* db, const std::string& query,
                         int (*callback)(void* data, int argc, char** argv,
                                         char**),
                         void* user_data) {
...
internal::index_t<
    T, decltype(internal::make_indexed_column(std::declval<Cols>()))...>
make_index(std::string name, Cols... cols) {
  using cols_tuple = std::tuple<Cols...>;
  static_assert(
      internal::count_tuple<cols_tuple, internal::is_where>::value <= 1,
      "amount of where arguments can be 0 or 1");
  SQLITE_ORM_CLANG_SUPPRESS_MISSING_BRACES(return {
      std::move(name), false,
      std::make_tuple(internal::make_indexed_column(std::move(cols))...)});
}

template <class... Cols>
internal::index_t<
    internal::table_type_of_t<
        typename std::tuple_element_t<0, std::tuple<Cols...>>>,
...
struct rowid_t {
  operator std::string() const { return "rowid"; }
};

struct oid_t {
  operator std::string() const { return "oid"; }
};

struct _rowid_t {
  operator std::string() const { return "_rowid_"; }
};
...
inline internal::rowid_t rowid() { return {}; }

inline internal::oid_t oid() { return {}; }

inline internal::_rowid_t _rowid_() { return {}; }
...
decltype(auto) empty_callable() {
  static auto res = [](auto&&...) -> R { return R(); };
  return (res);
}
...
void call_if_constexpr([[maybe_unused]] L&& lambda,
...
decltype(auto) static_if(T&& trueFn, F&& falseFn) {
  return static_if(std::integral_constant<bool, B>{}, std::forward<T>(trueFn),
                   std::forward<F>(falseFn));
}
...
auto call_impl(Function& f, FunctionPointer functionPointer, Tuple t,
...
auto call(Function& f, FunctionPointer functionPointer, Tuple t) {
  ...
  return call_impl(f, functionPointer, std::move(t),
...
void iterate_tuple(const Tpl& tpl, std::index_sequence<Idx...>, L&& lambda) {
  if constexpr (reversed) {
    // nifty fold expression trick: make use of guaranteed right-to-left
    // evaluation order when folding over operator=
    int sink;
...
void iterate_tuple(const Tpl& tpl, std::index_sequence<I, Idx...>, L&& lambda) {
  ...
  if (reversed) {
#endif
    iterate_tuple<reversed>(tpl, std::index_sequence<Idx...>{},
...
void iterate_tuple(const Tpl& tpl, L&& lambda) {
  iterate_tuple<reversed>(
      tpl, std::make_index_sequence<std::tuple_size<Tpl>::value>{},
      std::forward<L>(lambda));
}
...
void iterate_tuple(L&& lambda) {
  iterate_tuple<Tpl>(std::make_index_sequence<std::tuple_size<Tpl>::value>{},
                     std::forward<L>(lambda));
}

template <class R, class Tpl, size_t... Idx,
          class Projection = polyfill::identity>
R create_from_tuple(Tpl&& tpl, std::index_sequence<Idx...>,
                    Projection project = {}) {
...
R create_from_tuple(Tpl&& tpl, Projection project = {}) {
  return create_from_tuple<R>(
      ...
      std::make_index_sequence<
          std::tuple_size<std::remove_reference_t<Tpl>>::value>{},
      std::forward<Projection>(project));
}
 ...
 *  which in turn implicitly casts its single argument to the specified template
 * base class, then passes the converted argument to the lambda.
 *
 *  Note: This method is useful for reducing combinatorial instantiation of
 * template lambdas, as long as this library supports compilers that do not
 * implement explicit template parameters in generic lambdas
 * [SQLITE_ORM_EXPLICIT_GENERIC_LAMBDA_SUPPORTED]. Unfortunately it doesn't work
 * with user-defined conversion operators in order to extract parts of a class.
 * In other words, the destination type must be a direct template base class.
 */
template <template <class...> class Base, class L>
...
struct table_t : basic_table {
...
#if SQLITE_VERSION_NUMBER >= 3006019
    using fk_index_sequence =
        filter_tuple_sequence_t<elements_type, is_foreign_key>;
   ...
   *  so the method returns a pointer to the field as returned by the found
   * getter. Otherwise the method invokes the member pointer and returns its
   * result.
   */
  template <class M, satisfies_not<is_setter, M> = true>
  decltype(auto) object_field_value(const object_type& object,
                                    M memberPointer) const {
    return polyfill::invoke(memberPointer, object);
  }

  template <class M, satisfies<is_setter, M> = true>
  const member_field_type_t<M>* object_field_value(const object_type& object,
                                                   M memberPointer) const {
  ...
  find_column_generated_storage_type(const std::string& name) const {
    ...
    iterate_tuple(this->elements,
                  ...
                  [&result, &name](auto& column) {
                    ...
                    using generated_op_index_sequence = filter_tuple_sequence_t<
                        std::remove_const_t<decltype(column.constraints)>,
                        is_generated_always>;
                    constexpr size_t opIndex = first_index_sequence_value(
                        generated_op_index_sequence{});
  ...
  bool exists_in_composite_primary_key(const column_field<G, S>& column) const {
    ...
    this->for_each_primary_key([&column, &res](auto& primaryKey) {
      ...
      using same_type_index_sequence = filter_tuple_sequence_t<
          colrefs_tuple, check_if_is_type<member_field_type_t<G>>::template fn,
          member_field_type_t>;
      iterate_tuple(primaryKey.columns, same_type_index_sequence{},
                    [&res, &column](auto& memberPointer) {
  ...
  void for_each_primary_key(L&& lambda) const {
    using pk_index_sequence =
        filter_tuple_sequence_t<elements_type, is_primary_key>;
  ...
  std::vector<std::string> primary_key_column_names() const {
    ...
    if (pkcol_index_sequence::size() > 0) {
      return create_from_tuple<std::vector<std::string>>(
          this->elements, pkcol_index_sequence{}, &column_identifier::name);
    } else {
  ...
  void for_each_primary_key_column(L&& lambda) const {
    iterate_tuple(
        ...
        col_index_sequence_with<elements_type, is_primary_key>{},
        call_as_template_base<column_field>(
            [&lambda](const auto& column) { lambda(column.member_pointer); }));
  ...
  std::vector<std::string> composite_key_columns_names(
      const primary_key_t<Args...>& primaryKey) const {
    return create_from_tuple<std::vector<std::string>>(
        primaryKey.columns, [this, empty = std::string{}](auto& memberPointer) {
  ...
  /**
   *  Counts and returns amount of columns without GENERATED ALWAYS constraints.
   * Skips table constraints.
   */
  ...
  constexpr int count_columns_amount() const {
    using col_index_sequence =
        filter_tuple_sequence_t<elements_type, is_column>;
    return int(col_index_sequence::size());
  }

  /**
   *  Call passed lambda with all defined foreign keys.
   *  @param lambda Lambda called for each column. Function signature:
   * `void(auto& column)`
   */
  template <class L>
  ...
  void for_each_foreign_key_to(L&& lambda) const {
    ...
    using filtered_index_sequence =
        filter_tuple_sequence_t<elements_type,
                                ...
                                target_type_t, fk_index_sequence>;
  ...
  /**
   *  Call passed lambda with columns not having the specified constraint trait
   * `OpTrait`.
  ...
  void for_each_column_excluding(L&& lambda) const {
    iterate_tuple(this->elements,
                  col_index_sequence_excluding<elements_type, OpTraitFn>{},
                  lambda);
  }
  ...
  template <class OpTraitFnCls, class L,
            satisfies<mpl::is_metafunction_class, OpTraitFnCls> = true>
  void for_each_column_excluding(L&& lambda) const {
 ...
 *
 *  The mapped object type is determined implicitly from the first column
 * definition.
 */
template <class... Cs, class T = typename std::tuple_element_t<
                           0, std::tuple<Cs...>>::object_type>
internal::table_t<T, false, Cs...> make_table(std::string name, Cs... args) {
  SQLITE_ORM_CLANG_SUPPRESS_MISSING_BRACES(return {
      std::move(name), std::make_tuple<Cs...>(std::forward<Cs>(args)...)});
}
 ...
 *
 *  Note: unlike table_t<>, index_t<>::object_type and trigger_t<>::object_type
 * is always void.
 */
template <typename DBO, typename Lookup>
struct object_type_matches
    : polyfill::conjunction<
          polyfill::negation<std::is_void<object_type_t<DBO>>>,
          std::is_same<Lookup, object_type_t<DBO>>> {};

/**
 *  std::true_type if given lookup type (object) is mapped, std::false_type
 * otherwise.
 */
template <typename DBO, typename Lookup>
struct lookup_type_matches
    : polyfill::disjunction<object_type_matches<DBO, Lookup>> {};
}  // namespace internal
...
/**
 *   Indirect enabler for DBO, accepting an index to disambiguate non-unique
 * DBOs
 */
template <class Lookup, size_t Ix, class DBO>
struct enable_found_table
    : std::enable_if<lookup_type_matches<DBO, Lookup>::value, DBO> {};

/**
 *  SFINAE friendly facility to pick a table definition (`table_t`) from a tuple
 * of database objects.
 *
...
template <class Lookup, size_t... Ix, class... DBO>
struct storage_pick_table<Lookup, std::index_sequence<Ix...>,
                          db_objects_tuple<DBO...>>
...
using storage_pick_table_t = typename storage_pick_table<
    Lookup, std::make_index_sequence<std::tuple_size<DBOs>::value>,
 ...
 */
template <class Lookup, class DBOs>
struct storage_find_table
    : polyfill::detected_or<polyfill::nonesuch, storage_pick_table_t, Lookup,
                            DBOs> {};
 ...
 */
template <class Lookup, class DBOs>
using storage_find_table_t =
    typename storage_find_table<Lookup, std::remove_const_t<DBOs>>::type;

#ifndef SQLITE_ORM_BROKEN_VARIADIC_PACK_EXPANSION
template <class DBOs, class Lookup, class SFINAE = void>
struct is_mapped : std::false_type {};
template <class DBOs, class Lookup>
struct is_mapped<DBOs, Lookup,
                 polyfill::void_t<storage_pick_table_t<Lookup, DBOs>>>
    : std::true_type {};
#else
template <class DBOs, class Lookup,
          class SFINAE = storage_find_table_t<Lookup, DBOs>>
...
template <class DBOs, class Lookup>
SQLITE_ORM_INLINE_VAR constexpr bool is_mapped_v =
    is_mapped<DBOs, Lookup>::value;
}  // namespace internal
}  // namespace sqlite_orm
...
namespace internal {
/**
 *  Pick the table definition for the specified lookup type from the given tuple
 * of schema objects.
 *
 *  Note: This function requires Lookup to be mapped, otherwise it is removed
...
int foreign_keys_count(const DBOs& dbObjects) {
  int res = 0;
  iterate_tuple<true>(
      dbObjects, tables_index_sequence<DBOs>{},
...
auto lookup_table(const DBOs& dbObjects) {
  return static_if<is_mapped_v<DBOs, Lookup>>(
      [](const auto& dbObjects) { return &pick_table<Lookup>(dbObjects); },
 ...
 */
template <class O, class F, class DBOs, satisfies<is_db_objects, DBOs> = true>
constexpr decltype(auto) materialize_column_pointer(
    const DBOs&, const column_pointer<O, F>& cp) {
  return cp.field;
}
 ...
 */
template <class O, class F, class DBOs, satisfies<is_db_objects, DBOs> = true>
const std::string* find_column_name(const DBOs& dbObjects,
                                    const column_pointer<O, F>& cp) {
...
struct object_from_column_builder : object_from_column_builder_base {
  ...
  void operator()(const column_field<G, S>& column) {
    auto value = row_extractor<member_field_type_t<G>>().extract(this->stmt,
                                                                 this->index++);
 ...
 * by member pointers. To construct please use `make_row_extractor()`. Type
 * arguments: V is value type just like regular `row_extractor` has T is table
 * info class `table_t`
 */
...
/**
 *  If T is a recordset alias then the typename mapped_type_proxy<T>::type is
 * the unqualified aliased type, otherwise unqualified T.
 */
template <class T, class SFINAE = void>
struct mapped_type_proxy : std::remove_const<T> {};

template <class T>
struct mapped_type_proxy<T, match_if<is_recordset_alias, T>>
    : std::remove_const<type_t<T>> {};
...
struct storage_mapped_columns_impl
    : tuple_transformer<filter_tuple_t<elements_type_t<DBO>, is_column>,
                        field_type_t> {};
...
struct storage_mapped_columns
    : storage_mapped_columns_impl<storage_find_table_t<Lookup, DBOs>> {};
}  // namespace storage_traits
}  // namespace internal
}  // namespace sqlite_orm
...
struct user_defined_function_base {
  using func_call =
      std::function<void(sqlite3_context* context, void* functionPointer,
                         int argsCount, sqlite3_value** values)>;
  using final_call =
      std::function<void(sqlite3_context* context, void* functionPointer)>;
...
struct user_defined_scalar_function_t : user_defined_function_base {
  ...
  user_defined_scalar_function_t(decltype(name) name_, int argumentsCount_,
                                 decltype(create) create_, decltype(run) run_,
                                 decltype(destroy) destroy_)
      : user_defined_function_base{std::move(name_), argumentsCount_,
                                   std::move(create_), destroy_},
        run(std::move(run_)) {}
};
...
template <class F>
SQLITE_ORM_INLINE_VAR constexpr bool
    is_scalar_function_v<F, polyfill::void_t<scalar_call_function_t<F>>> = true;
...
SQLITE_ORM_INLINE_VAR constexpr bool is_aggregate_function_v<
    F,
    polyfill::void_t<aggregate_step_function_t<F>, aggregate_fin_function_t<F>,
                     std::enable_if_t<std::is_member_function_pointer<
                         aggregate_step_function_t<F>>::value>,
                     std::enable_if_t<std::is_member_function_pointer<
                         aggregate_fin_function_t<F>>::value>>> = true;
...
struct callable_arguments_impl<F, std::enable_if_t<is_scalar_function_v<F>>> {
  using args_tuple =
      typename member_function_arguments<scalar_call_function_t<F>>::tuple_type;
  using return_type = typename member_function_arguments<
      scalar_call_function_t<F>>::return_type;
};

template <class F>
struct callable_arguments_impl<F,
                               std::enable_if_t<is_aggregate_function_v<F>>> {
  using args_tuple = typename member_function_arguments<
      aggregate_step_function_t<F>>::tuple_type;
  using return_type = typename member_function_arguments<
      aggregate_fin_function_t<F>>::return_type;
};
...
SQLITE_ORM_CONSTEVAL bool expected_pointer_value() {
  static_assert(polyfill::always_false_v<FnArg, CallArg>,
                "Expected a pointer value for I-th argument");
  return false;
}
...
constexpr bool is_same_pvt_v<I, PointerArg, nullptr_t,
                             polyfill::void_t<typename PointerArg::tag>> = true;
...
constexpr bool is_same_pvt_v<
    I, PointerArg, Binding,
    polyfill::void_t<typename PointerArg::tag, typename Binding::tag>> =
...
constexpr bool is_same_pvt_v<
    I, PointerArg, Binding,
    polyfill::void_t<typename PointerArg::tag, typename Binding::tag>> =
    assert_same_pointer_type<I, typename PointerArg::tag,
                             typename Binding::tag>();
#endif
...
SQLITE_ORM_CONSTEVAL bool validate_pointer_value_types(
    polyfill::index_constant<size_t(-1)>) {
  return true;
}
template <class FnArgs, class CallArgs, size_t I>
SQLITE_ORM_CONSTEVAL bool validate_pointer_value_types(
    polyfill::index_constant<I>) {
  ...
  constexpr bool valid = validate_pointer_value_type<
      I, std::tuple_element_t<I, FnArgs>,
      unpacked_arg_t<std::tuple_element_t<I, CallArgs>>>(
      polyfill::bool_constant <
          (polyfill::is_specialization_of_v<func_arg_t, pointer_arg>) ||
      (polyfill::is_specialization_of_v<passed_arg_t, pointer_binding>) > {});

  return validate_pointer_value_types<FnArgs, CallArgs>(
             polyfill::index_constant<I - 1>{}) &&
         valid;
#else
  return validate_pointer_value_types<FnArgs, CallArgs>(
             polyfill::index_constant<I - 1>{}) &&
         validate_pointer_value_type<
             I, std::tuple_element_t<I, FnArgs>,
             unpacked_arg_t<std::tuple_element_t<I, CallArgs>>>(
             polyfill::bool_constant <
                 (polyfill::is_specialization_of_v<func_arg_t, pointer_arg>) ||
...
internal::function_call<F, Args...> func(Args... args) {
  using args_tuple = std::tuple<Args...>;
  using function_args_tuple =
      typename internal::callable_arguments<F>::args_tuple;
  constexpr auto argsCount = std::tuple_size<args_tuple>::value;
  constexpr auto functionArgsCount =
      std::tuple_size<function_args_tuple>::value;
  static_assert(
      (argsCount == functionArgsCount &&
       ...
       internal::validate_pointer_value_types<function_args_tuple, args_tuple>(
           polyfill::index_constant<std::min<>(functionArgsCount, argsCount) -
                                    1>{})) ||
          std::is_same<function_args_tuple, std::tuple<arg_values>>::value,
...
/**
 *  Obtains the result type of expressions that form the columns of a select
 * statement.
 *
 ...
 * type for T DBOs - db_objects_tuple type T - C++ type SFINAE - sfinae argument
 */
...
template <class DBOs, class T>
struct column_result_t<DBOs, T, match_if<std::is_member_pointer, T>>
    : member_field_type<T> {};
...
template <class DBOs, class R, class S, class... Args>
struct column_result_t<DBOs, built_in_aggregate_function_t<R, S, Args...>,
                       void> {
  using type = R;
};
...
struct column_result_t<
    DBOs, built_in_function_t<unique_ptr_result_of<X>, S, X, Rest...>, void> {
  using type = std::unique_ptr<column_result_of_t<DBOs, X>>;
};

template <class DBOs, class X, class S>
struct column_result_t<
    DBOs, built_in_aggregate_function_t<unique_ptr_result_of<X>, S, X>, void> {
  using type = std::unique_ptr<column_result_of_t<DBOs, X>>;
};
...
struct column_result_t<DBOs, alias_column_t<T, C>, void>
    : column_result_t<DBOs, C> {};

template <class DBOs, class T, class F>
struct column_result_t<DBOs, column_pointer<T, F>, void>
    : column_result_t<DBOs, F> {};

template <class DBOs, class... Args>
struct column_result_t<DBOs, columns_t<Args...>, void> {
  using type =
      tuple_cat_t<tuplify_t<column_result_of_t<DBOs, std::decay_t<Args>>>...>;
};
...
struct column_result_t<DBOs, as_t<T, E>, void>
    : column_result_t<DBOs, std::decay_t<E>> {};
...
template <class DBOs, class T>
struct column_result_t<DBOs, std::reference_wrapper<T>, void>
    : column_result_t<DBOs, T> {};
}  // namespace internal
}  // namespace sqlite_orm
...
struct iterator_t {
 ...
 protected:
  ...
  void extract_value() {
    ...
    object_from_column_builder<value_type> builder{*this->current,
                                                   this->stmt.get()};
...
struct connection_holder {
  ...
  sqlite3* get() const { return this->db; }

  int retain_count() const { return this->_retain_count; }
...
struct connection_ref {
  ...
  ~connection_ref() { this->holder.release(); }

  sqlite3* get() const { return this->holder.get(); }
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_values_v =
    polyfill::is_specialization_of_v<T, values_t>;
...
struct conflict_target {
  ...
  upsert_clause<args_tuple, std::tuple<ActionsArgs...>> do_update(
      ActionsArgs... actions) {
 ...
 *            columns(&Employee::id, &Employee::name, &Employee::age,
 * &Employee::address, &Employee::salary), values(std::make_tuple(3, "Sofia",
 * 26, "Madrid", 15000.0), std::make_tuple(4, "Doja", 26, "LA", 25000.0)),
 *            on_conflict(&Employee::id).do_update(set(c(&Employee::name) =
 * excluded(&Employee::name), c(&Employee::age) = excluded(&Employee::age),
 *                                           c(&Employee::address) =
 * excluded(&Employee::address), c(&Employee::salary) =
 * excluded(&Employee::salary))));
 */
template <class... Args>
...
struct table_name_collector : table_name_collector_base {
  ...
  void operator()(const alias_column_t<A, C>&) {
    // note: instead of accessing the column, we are interested in the type the
    // column is aliased into
    auto tableName =
        lookup_table_name<mapped_type_proxy_t<A>>(this->db_objects);
    this->table_names.emplace(std::move(tableName),
                              alias_extractor<A>::as_alias());
  }
  ...
  void operator()(const asterisk_t<T>&) {
    auto tableName =
        lookup_table_name<mapped_type_proxy_t<T>>(this->db_objects);
...
/**
 *  SET keyword used in UPDATE ... SET queries. It is dynamic version. It means
 * use can add amount of arguments now known at compilation time but known at
 * runtime.
 */
template <class S>
internal::dynamic_set_t<
    internal::serializer_context<typename S::db_objects_type>>
dynamic_set(const S& storage) {
  internal::serializer_context_builder<S> builder(storage);
  return builder();
}
}  // namespace sqlite_orm
...
struct prepared_statement_base {
  ...
  ~prepared_statement_base() { sqlite3_finalize(this->stmt); }

  std::string sql() const {
    // note: sqlite3 internally checks for null before calling
    // sqlite3_normalized_sql() or sqlite3_expanded_sql(), so check here, too,
    // even if superfluous
  ...
  std::string expanded_sql() const {
    ...
    using char_ptr = std::unique_ptr<
        char, std::integral_constant<decltype(&sqlite3_free), sqlite3_free>>;
...
struct prepared_statement_t : prepared_statement_base {
  ...
  prepared_statement_t(T expression_, sqlite3_stmt* stmt_, connection_ref con_)
      : prepared_statement_base{stmt_, std::move(con_)},
        expression(std::move(expression_)) {}

  prepared_statement_t(prepared_statement_t&& prepared_stmt)
      : prepared_statement_base{prepared_stmt.stmt,
                                std::move(prepared_stmt.con)},
...
template <class T>
using is_prepared_statement =
    polyfill::bool_constant<is_prepared_statement_v<T>>;
...
struct update_all_t {
  ...
  static_assert(
      is_set<S>::value,
      "update_all_t must have set or dynamic set as the first argument");
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_insert_v =
    polyfill::is_specialization_of_v<T, insert_t>;
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_replace_v =
    polyfill::is_specialization_of_v<T, replace_t>;
...
SQLITE_ORM_INLINE_VAR constexpr bool is_insert_range_v =
    polyfill::is_specialization_of_v<T, insert_range_t>;
...
SQLITE_ORM_INLINE_VAR constexpr bool is_replace_range_v =
    polyfill::is_specialization_of_v<T, replace_range_t>;
...
template <class T>
SQLITE_ORM_INLINE_VAR constexpr bool is_insert_raw_v =
    polyfill::is_specialization_of_v<T, insert_raw_t>;
...
SQLITE_ORM_INLINE_VAR constexpr bool is_replace_raw_v =
    polyfill::is_specialization_of_v<T, replace_raw_t>;
 ...
 *  ```
 */
inline internal::default_values_t default_values() { return {}; }

/**
 *  Raw insert statement creation routine. Use this if `insert` with object does
 * not fit you. This insert is designed to be able to call any type of `INSERT`
 * query with no limitations.
 *  @example
 *  ```sql
 ...
 *  will be
 *  ```c++
 *  auto statement = storage.prepare(insert(into<User>, columns(&User::id,
 * &User::name), values(std::make_tuple(5, "Little Mix"))));
 ...
 *  will be
 *  ```c++
 *  auto statement = storage.prepare(insert(into<Singer>(),
 * columns(&Singer::name), values(std::make_tuple("Sofia Reyes"),
 * std::make_tuple("Kungs")))); storage.execute(statement));
 *  ```
 ...
 *  storage.execute(statement));
 *  ```
 *  Also one can use `INSERT OR ABORT`/`INSERT OR FAIL`/`INSERT OR
 * IGNORE`/`INSERT OR REPLACE`/`INSERT ROLLBACK`:
 *  ```c++
 *  auto statement = storage.prepare(insert(or_ignore(), into<Singer>(),
 * columns(&Singer::name), values(std::make_tuple("Sofia Reyes"),
 * std::make_tuple("Kungs")))); auto statement2 =
 * storage.prepare(insert(or_rollback(), into<Singer>(), default_values()));
 *  auto statement3 = storage.prepare(insert(or_abort(), into<User>,
 * columns(&User::id, &User::name), values(std::make_tuple(5, "Little Mix"))));
 *  ```
 */
template <class... Args>
internal::insert_raw_t<Args...> insert(Args... args) {
  ...
  constexpr int orArgsCount =
      count_tuple<args_tuple, is_insert_constraint>::value;
  static_assert(orArgsCount < 2,
                "Raw insert must have only one OR... argument");
  ...
  static_assert(intoArgsCount < 2,
                "Raw insert must have only one into<T> argument");

  constexpr int columnsArgsCount = count_tuple<args_tuple, is_columns>::value;
  static_assert(columnsArgsCount < 2,
                "Raw insert must have only one columns(...) argument");

  constexpr int valuesArgsCount = count_tuple<args_tuple, is_values>::value;
  static_assert(valuesArgsCount < 2,
                "Raw insert must have only one values(...) argument");

  constexpr int defaultValuesCount =
      count_tuple<args_tuple, internal::is_default_values>::value;
  static_assert(defaultValuesCount < 2,
                "Raw insert must have only one default_values() argument");

  constexpr int selectsArgsCount = count_tuple<args_tuple, is_select>::value;
  static_assert(selectsArgsCount < 2,
                "Raw insert must have only one select(...) argument");

  constexpr int upsertClausesCount =
      count_tuple<args_tuple, is_upsert_clause>::value;
  static_assert(upsertClausesCount <= 2,
                "Raw insert can contain 2 instances of upsert clause maximum");

  constexpr int argsCount = int(std::tuple_size<args_tuple>::value);
  static_assert(argsCount == intoArgsCount + columnsArgsCount +
                                 valuesArgsCount + defaultValuesCount +
...
/**
 *  Raw replace statement creation routine. Use this if `replace` with object
 * does not fit you. This replace is designed to be able to call any type of
 * `REPLACE` query with no limitations. Actually this is the same query as raw
 * insert except `OR...` option existance.
 *  @example
 *  ```sql
 ...
 *  will be
 *  ```c++
 *  auto statement = storage.prepare(replace(into<User>, columns(&User::id,
 * &User::name), values(std::make_tuple(5, "Little Mix"))));
 *  storage.execute(statement));
 *  ```
 *  One more example:
 ...
 *  will be
 *  ```c++
 *  auto statement = storage.prepare(replace(into<Singer>(),
 * columns(&Singer::name), values(std::make_tuple("Sofia Reyes"),
 * std::make_tuple("Kungs")))); storage.execute(statement));
 *  ```
 *  One can use `default_values` to add `DEFAULT VALUES` modifier:
...
internal::replace_raw_t<Args...> replace(Args... args) {
  ...
  static_assert(intoArgsCount < 2,
                "Raw replace must have only one into<T> argument");

  constexpr int columnsArgsCount = count_tuple<args_tuple, is_columns>::value;
  static_assert(columnsArgsCount < 2,
                "Raw replace must have only one columns(...) argument");

  constexpr int valuesArgsCount = count_tuple<args_tuple, is_values>::value;
  static_assert(valuesArgsCount < 2,
                "Raw replace must have only one values(...) argument");

  constexpr int defaultValuesCount =
      count_tuple<args_tuple, internal::is_default_values>::value;
  static_assert(defaultValuesCount < 2,
                "Raw replace must have only one default_values() argument");

  constexpr int selectsArgsCount =
      count_tuple<args_tuple, internal::is_select>::value;
  static_assert(selectsArgsCount < 2,
                "Raw replace must have only one select(...) argument");
 ...
 *  The objects in the range are transformed using the specified projection,
 * which defaults to identity projection.
 *
 *  @example
 *  ```
 ...
 *  auto statement = storage.prepare(replace_range(userPointers.begin(),
 * userPointers.end(), &std::unique_ptr<User>::operator*));
 *  storage.execute(statement);
 *  ```
 */
template <class It, class Projection = polyfill::identity>
auto replace_range(It from, It to, Projection project = {}) {
  using O = std::decay_t<decltype(polyfill::invoke(std::declval<Projection>(),
                                                   *std::declval<It>()))>;
  return internal::replace_range_t<It, Projection, O>{
      {std::move(from), std::move(to)}, std::move(project)};
}

/*
 *  Create a replace range statement.
 *  Overload of `replace_range(It, It, Projection)` with explicit object type
 * template parameter.
 */
template <class O, class It, class Projection = polyfill::identity>
internal::replace_range_t<It, Projection, O> replace_range(
    It from, It to, Projection project = {}) {
 ...
 *  std::vector<std::unique_ptr<User>> userPointers;
 *  userPointers.push_back(std::make_unique<User>(1, "Eneli"));
 *  auto statement = storage.prepare(insert_range(userPointers.begin(),
 * userPointers.end(), &std::unique_ptr<User>::operator*));
 *  storage.execute(statement);
 *  ```
 */
...
/*
 *  Create an insert range statement.
 *  Overload of `insert_range(It, It, Projection)` with explicit object type
 * template parameter.
 */
template <class O, class It, class Projection = polyfill::identity>
internal::insert_range_t<It, Projection, O> insert_range(
    It from, It to, Projection project = {}) {
  return {{std::move(from), std::move(to)}, std::move(project)};
}
 ...
 */
template <class T, class... Cols>
internal::insert_explicit<T, Cols...> insert(
    T obj, internal::columns_t<Cols...> cols) {
 ...
 *  Column expressions can be more complex, but are passed as a whole to the
 * callable. Examples are `column_pointer<>` and `alias_column_t<>`.
 *
...
struct ast_iterator<std::tuple<Args...>, void> {
  ...
  template <class L>
  void operator()(const node_type& node, L& lambda) const {
    iterate_tuple(node, [&lambda](auto& v) { iterate_ast(v, lambda); });
  }
};
...
struct ast_iterator<like_t<A, T, E>, void> {
  ...
  void operator()(const node_type& lk, L& lambda) const {
    ...
    lk.arg3.apply([&lambda](auto& value) { iterate_ast(value, lambda); });
  }
};
...
template <class T>
struct ast_iterator<
    T, std::enable_if_t<polyfill::is_specialization_of_v<T, using_t>>> {
  using node_type = T;
...
struct ast_iterator<simple_case_t<R, T, E, Args...>, void> {
  ...
  template <class L>
  void operator()(const node_type& c, L& lambda) const {
    c.case_expression.apply([&lambda](auto& c_) { iterate_ast(c_, lambda); });
    iterate_tuple(c.args, [&lambda](auto& pair) {
      iterate_ast(pair.first, lambda);
      iterate_ast(pair.second, lambda);
    });
    c.else_expression.apply([&lambda](auto& el) { iterate_ast(el, lambda); });
  }
};
...
struct ast_iterator<limit_t<T, true, false, O>, void> {
  ...
  void operator()(const node_type& a, L& lambda) const {
    iterate_ast(a.lim, lambda);
    a.off.apply([&lambda](auto& value) { iterate_ast(value, lambda); });
  }
};
 ...
 */
template <class T>
struct ast_iterator<T, std::enable_if_t<polyfill::disjunction_v<
                           polyfill::is_specialization_of<T, alias_holder>,
...
/**
 * This class does not related to SQL view. This is a container like class which
 * is returned by by storage_t::iterate function. This class contains STL
 * functions:
 *  -   size_t size()
 *  -   bool empty()
 *  -   iterator end()
 *  -   iterator begin()
 *  All these functions are not right const cause all of them may open SQLite
 * connections.
 */
...
inline void stream_sql_escaped(std::ostream& os, serialize_arg_type str,
                               char char2Escape) {
...
inline void stream_identifier(std::ostream& ss, serialize_arg_type qualifier,
  ...
  // note: In practice, escaping double quotes in identifiers is arguably
  // overkill, but since the SQLite grammar allows it, it's better to be safe
  // than sorry.
...
template <
    typename Tpl,
    std::enable_if_t<polyfill::is_detected_v<type_t, std::tuple_size<Tpl>>,
                     bool> = true>
void stream_identifier(std::ostream& ss, const Tpl& tpl) {
  return stream_identifier(
      ss, tpl, std::make_index_sequence<std::tuple_size<Tpl>::value>{});
}
...
struct streaming {
  ...
  template <size_t... Idx>
  constexpr std::index_sequence<1u + Idx...> offset_index(
      std::index_sequence<Idx...>) const {
    return {};
  }
};
...
constexpr streaming<stream_as::dynamic_expressions>
    streaming_dynamic_expressions{};
...
constexpr streaming<stream_as::values_placeholders>
    streaming_values_placeholders{};
constexpr streaming<stream_as::table_columns> streaming_table_column_names{};
constexpr streaming<stream_as::non_generated_columns>
    streaming_non_generated_column_names{};
constexpr streaming<stream_as::field_values_excluding>
    streaming_field_values_excluding{};
constexpr streaming<stream_as::mapped_columns_expressions>
    streaming_mapped_columns_expressions{};
constexpr streaming<stream_as::column_constraints>
...
std::ostream& operator<<(
    std::ostream& ss,
    std::tuple<const streaming<stream_as::actions_tuple>&, T, Ctx> tpl) {
...
std::ostream& operator<<(
    std::ostream& ss, std::tuple<const streaming<stream_as::expressions_tuple>&,
                                 const std::tuple<order_by_t<Os>...>&, Ctx>
                          tpl) {
  const auto& args = get<1>(tpl);
  auto& context = get<2>(tpl);
...
std::ostream& operator<<(
    std::ostream& ss,
    std::tuple<const streaming<stream_as::serialized>&, C> tpl) {
...
std::ostream& operator<<(
  ...
  stream_identifier(
      ss, tpl,
      streaming_identifier.offset_index(std::index_sequence_for<Strings...>{}));
  return ss;
}

// stream a container of identifiers described by a string or a tuple, which is
// one of:
// 1. identifier
// 1. tuple(identifier)
...
std::ostream& operator<<(
    std::ostream& ss,
    std::tuple<const streaming<stream_as::identifiers>&, C> tpl) {
...
std::ostream& operator<<(
  ...
  std::string result;
  result.reserve((1 + (columnsCount * 1) + (columnsCount * 2 - 2) + 1) *
                     valuesCount +
                 (valuesCount * 2 - 2));
...
std::ostream& operator<<(
    std::ostream& ss,
    std::tuple<const streaming<stream_as::table_columns>&, Table, const bool&>
        tpl) {
  const auto& table = get<1>(tpl);
  const bool& qualified = get<2>(tpl);

  table.for_each_column(
      [&ss, &tableName = qualified ? table.name : std::string{},
       first = true](const column_identifier& column) mutable {
        constexpr std::array<const char*, 2> sep = {", ", ""};
        ss << sep[std::exchange(first, false)];
...
std::ostream& operator<<(
    std::ostream& ss,
    std::tuple<const streaming<stream_as::field_values_excluding>&, PredFnCls,
               L, Ctx, Obj>
        tpl) {
  using check_if_excluded =
      polyfill::remove_cvref_t<std::tuple_element_t<1, decltype(tpl)>>;
  ...
  table.template for_each_column_excluding<check_if_excluded>(
      call_as_template_base<column_field>([&ss, &excluded, &context, &object,
                                           first = true](auto& column) mutable {
        if (excluded(column)) {
          return;
        }
        ...
        constexpr std::array<const char*, 2> sep = {", ", ""};
        ss << sep[std::exchange(first, false)]
           << serialize(polyfill::invoke(column.member_pointer, object),
                        context);
      }));
  return ss;
}

// stream a tuple of mapped columns (which are member pointers or column
// pointers); comma-separated
template <class T, class Ctx>
std::ostream& operator<<(
    std::ostream& ss,
    std::tuple<const streaming<stream_as::mapped_columns_expressions>&, T, Ctx>
        tpl) {
  ...
  iterate_tuple(columns, [&ss, &context, first = true](auto& colRef) mutable {
    const std::string* columnName =
        find_column_name(context.db_objects, colRef);
    if (!columnName) {
...
std::ostream& operator<<(
    ...
    std::tuple<const streaming<stream_as::column_constraints>&,
               const column_constraints<Op...>&, const bool&, Ctx>
        tpl) {
  ...
  if (constraintsCount) {
    ...
    int tupleIndex = 0;
    iterate_tuple(column.constraints, [&constraintsStrings, &primaryKeyIndex,
                                       &autoincrementIndex, &tupleIndex,
                                       &context](auto& constraint) {
...
template <>
inline int getPragmaCallback<std::vector<std::string>>(void* data, int argc,
                                                       char** argv, char**) {
...
struct pragma_t {
  ...
  void busy_timeout(int value) { this->set_pragma("busy_timeout", value); }

  int busy_timeout() { return this->get_pragma<int>("busy_timeout"); }
   ...
   */
  int application_id() { return this->get_pragma<int>("application_id"); }

  /**
   *  https://www.sqlite.org/pragma.html#pragma_application_id
   */
  void application_id(int value) { this->set_pragma("application_id", value); }
  ...
  std::vector<sqlite_orm::table_xinfo> table_xinfo(
    ...
    perform_exec(
        ...
        [](void* data, int argc, char** argv, char**) -> int {
          ...
          if (argc) {
            ...
            res.emplace_back(cid, std::move(name), std::move(type), notnull,
                             std::move(dflt_value), pk, hidden);
          }
          return 0;
        },
        &result);
  ...
  std::vector<sqlite_orm::table_info> table_info(
      const std::string& tableName) const {
    auto connection = this->get_connection();
 ...
 private:
  ...
  T get_pragma(const std::string& name) {
    auto connection = this->get_connection();
    T result;
    perform_exec(connection.get(), "PRAGMA " + name, getPragmaCallback<T>,
                 &result);
    return result;
  }

  /**
   *  Yevgeniy Zakharov: I wanted to refactor this function with statements and
  ...
  void set_pragma(const std::string& name,
 ...
 *  so exceptions that occur during commit or rollback are propagated to the
 * caller.
 */
struct transaction_guard_t {
  ...
  transaction_guard_t(transaction_guard_t&& other)
      : commit_on_destroy(other.commit_on_destroy),
 ...
 */
struct backup_t {
  backup_t(connection_ref to_, const std::string& zDestName,
           connection_ref from_, const std::string& zSourceName,
           std::unique_ptr<connection_holder> holder_)
      : handle(sqlite3_backup_init(to_.get(), zDestName.c_str(), from_.get(),
                                   zSourceName.c_str())),
   ...
   */
  int step(int pages) { return sqlite3_backup_step(this->handle, pages); }

  /**
   *  Returns sqlite3_backup_remaining result
   */
  int remaining() const { return sqlite3_backup_remaining(this->handle); }

  /**
   *  Returns sqlite3_backup_pagecount result
   */
  int pagecount() const { return sqlite3_backup_pagecount(this->handle); }
...
struct arg_value {
  ...
  bool empty() const { return this->value == nullptr; }
...
struct arg_values {
  struct iterator {
    iterator(const arg_values& container_, int index_)
        : container(container_),
          index(index_),
          currentValue(index_ < int(container_.size()) ? container_[index_]
                                                       : arg_value()) {}
    ...
    arg_value* operator->() const { return &this->currentValue; }
  ...
  size_t size() const { return this->argsCount; }

  bool empty() const { return 0 == this->argsCount; }
  ...
  arg_value at(int index) const { return this->operator[](index); }

  iterator begin() const { return {*this, 0}; }

  iterator end() const { return {*this, this->argsCount}; }
...
struct values_to_tuple {
  ...
  void operator()(sqlite3_value** values, std::tuple<arg_values>& tuple,
                  int argsCount) const {
...
#ifdef SQLITE_ORM_FOLD_EXPRESSIONS_SUPPORTED
  template <class Tpl, size_t... Idx>
  void operator()(sqlite3_value** values, Tpl& tuple,
                  std::index_sequence<Idx...>) const {
...
struct storage_base {
  ...
  transaction_guard_t transaction_guard() {
    ...
    return {this->get_connection(), std::bind(&storage_base::commit, this),
  ...
  void vacuum() { perform_void_exec(this->get_connection().get(), "VACUUM"); }
 ...
 protected:
  void rename_table(sqlite3* db, const std::string& oldName,
 ...
 public:
  ...
  std::vector<std::string> table_names() {
    ...
    perform_exec(
        con.get(), "SELECT name FROM sqlite_master WHERE type='table'",
  ...
  /**
   * Call this to create user defined scalar function. Can be called at any time
   * no matter connection is opened or no. T - function class. T must have
   * operator() overload and static name function like this:
   * ```
   *  struct SqrtFunction {
   *
   ...
   * ```
   *
   * Note: Currently, a function's name must not contain white-space characters,
   * because it doesn't get quoted.
   */
  template <class F>
  void create_scalar_function() {
    ...
    constexpr auto argsCount =
        std::is_same<args_tuple, std::tuple<arg_values>>::value
            ? -1
            : int(std::tuple_size<args_tuple>::value);
    this->scalarFunctions.emplace_back(new user_defined_scalar_function_t{
        std::move(name),
        argsCount,
        []() -> int* { return (int*)(new F()); },
        /* call = */
        [](sqlite3_context* context, void* functionVoidPointer, int argsCount,
    ...
    if (this->connection->retain_count() > 0) {
      ...
      try_to_create_function(db, static_cast<user_defined_scalar_function_t&>(
                                     *this->scalarFunctions.back()));
    }
  }

  /**
   * Call this to create user defined aggregate function. Can be called at any
   * time no matter connection is opened or no. T - function class. T must have
   * step member function, fin member function and static name function like
   * this:
   * ```
   *   struct MeanFunction {
   *       double total = 0;
  ...
  void create_aggregate_function() {
    ...
    this->aggregateFunctions.emplace_back(new user_defined_aggregate_function_t{
        ...
        []() -> int* { return (int*)(new F()); },
        /* step = */
        [](sqlite3_context*, void* functionVoidPointer, int argsCount,
           sqlite3_value** values) {
          auto& function = *static_cast<F*>(functionVoidPointer);
          args_tuple argsTuple;
          values_to_tuple{}(values, argsTuple, argsCount);
    ...
    if (this->connection->retain_count() > 0) {
      ...
      try_to_create_function(db,
                             static_cast<user_defined_aggregate_function_t&>(
                                 *this->aggregateFunctions.back()));
    }
  }

  /**
   *  Use it to delete scalar function you created before. Can be called at any
   * time no matter connection is open or no.
   */
  template <class F>
  ...
  /**
   *  Use it to delete aggregate function you created before. Can be called at
   * any time no matter connection is open or no.
   */
  template <class F>
  ...
  void create_collation() {
    collating_function func = [](int leftLength, const void* lhs,
                                 int rightLength, const void* rhs) {
      C collatingObject;
  ...
  void create_collation(const std::string& name, collating_function f) {
    ...
    if (this->connection->retain_count() > 0) {
      sqlite3* db = this->connection->get();
      auto resultCode =
          sqlite3_create_collation(db, name.c_str(), SQLITE_UTF8, function,
  ...
  const std::string& filename() const { return this->connection->filename; }

  /**
   * Checks whether connection to database is opened right now.
   * Returns always `true` for in memory databases.
   */
  bool is_opened() const { return this->connection->retain_count() > 0; }

  /*
   * returning false when there is a transaction in place
   * otherwise true; function is not const because it has to call
   * get_connection()
   */
  bool get_autocommit() {
    auto con = this->get_connection();
  ...
  int busy_handler(std::function<int(int)> handler) {
    ...
    if (this->is_opened()) {
      if (_busy_handler) {
        return sqlite3_busy_handler(this->connection->get(),
                                    busy_handler_callback, this);
      } else {
 ...
 protected:
  ...
  bool foreign_keys(sqlite3* db) {
    bool result = false;
    perform_exec(db, "PRAGMA foreign_keys", extract_single_value<bool>,
                 &result);
    return result;
  }
  ...
  void on_open_internal(sqlite3* db) {
    ...
    if (this->pragma._journal_mode != -1) {
      this->pragma.set_pragma(
          "journal_mode", static_cast<journal_mode>(this->pragma._journal_mode),
          db);
    }

    for (auto& p : this->collatingFunctions) {
      auto resultCode = sqlite3_create_collation(
    ...
    if (_busy_handler) {
      sqlite3_busy_handler(this->connection->get(), busy_handler_callback,
                           this);
    }

    for (auto& functionPointer : this->scalarFunctions) {
      try_to_create_function(
          db, static_cast<user_defined_scalar_function_t&>(*functionPointer));
    }

    for (auto& functionPointer : this->aggregateFunctions) {
      try_to_create_function(
          db,
          static_cast<user_defined_aggregate_function_t&>(*functionPointer));
    }
  ...
  void delete_function_impl(
      ...
      std::vector<std::unique_ptr<user_defined_function_base>>& functionsVector)
      const {
    auto it = find_if(functionsVector.begin(), functionsVector.end(),
                      [&name](auto& functionPointer) {
    ...
    if (it != functionsVector.end()) {
      ...
      if (this->connection->retain_count() > 0) {
        sqlite3* db = this->connection->get();
        auto resultCode = sqlite3_create_function_v2(
            db, name.c_str(), 0, SQLITE_UTF8, nullptr, nullptr, nullptr,
            nullptr, nullptr);
        if (resultCode != SQLITE_OK) {
          throw_translated_sqlite_error(db);
        }
  ...
  void try_to_create_function(sqlite3* db,
                              user_defined_scalar_function_t& function) {
    auto resultCode = sqlite3_create_function_v2(
        db, function.name.c_str(), function.argumentsCount, SQLITE_UTF8,
        &function, scalar_function_callback, nullptr, nullptr, nullptr);
    if (resultCode != SQLITE_OK) {
      throw_translated_sqlite_error(db);
    }
  }
  ...
  void try_to_create_function(sqlite3* db,
                              user_defined_aggregate_function_t& function) {
    auto resultCode = sqlite3_create_function(
        db, function.name.c_str(), function.argumentsCount, SQLITE_UTF8,
        &function, nullptr, aggregate_function_step_callback,
  ...
  static void aggregate_function_step_callback(sqlite3_context* context,
                                               int argsCount,
                                               sqlite3_value** values) {
    auto functionVoidPointer = sqlite3_user_data(context);
    auto functionPointer =
        static_cast<user_defined_aggregate_function_t*>(functionVoidPointer);
    auto aggregateContextVoidPointer =
        sqlite3_aggregate_context(context, sizeof(int**));
    auto aggregateContextIntPointer =
    ...
    functionPointer->step(context, *aggregateContextIntPointer, argsCount,
                          values);
  }
  ...
  static void scalar_function_callback(sqlite3_context* context, int argsCount,
                                       sqlite3_value** values) {
    auto functionVoidPointer = sqlite3_user_data(context);
    auto functionPointer =
        static_cast<user_defined_scalar_function_t*>(functionVoidPointer);
    std::unique_ptr<int, void (*)(int*)> callablePointer(
        functionPointer->create(), functionPointer->destroy);
    if (functionPointer->argumentsCount != -1 &&
        functionPointer->argumentsCount != argsCount) {
  ...
  std::string current_timestamp(sqlite3* db) {
    std::string result;
    perform_exec(db, "SELECT CURRENT_TIMESTAMP",
                 extract_single_value<std::string>, &result);
    return result;
  }
  ...
  static int collate_callback(void* arg, int leftLen, const void* lhs,
                              int rightLen, const void* rhs) {
  ...
  bool calculate_remove_add_columns(
    ...
    for (size_t storageColumnInfoIndex = 0;
      ...
      auto dbColumnInfoIt = std::find_if(
          dbTableInfo.begin(), dbTableInfo.end(),
          [&columnName](auto& ti) { return ti.name == columnName; });
      if (dbColumnInfoIt != dbTableInfo.end()) {
        auto& dbColumnInfo = *dbColumnInfoIt;
        auto columnsAreEqual =
            dbColumnInfo.name == storageColumnInfo.name &&
            dbColumnInfo.notnull == storageColumnInfo.notnull &&
            (!dbColumnInfo.dflt_value.empty()) ==
                (!storageColumnInfo.dflt_value.empty()) &&
...
template <class It, class L, class O>
struct expression_object_type<
    replace_range_t<std::reference_wrapper<It>, L, O>> {
  using type =
      typename replace_range_t<std::reference_wrapper<It>, L, O>::object_type;
};
...
template <class It, class L, class O>
struct expression_object_type<
    insert_range_t<std::reference_wrapper<It>, L, O>> {
  using type =
      typename insert_range_t<std::reference_wrapper<It>, L, O>::object_type;
};
...
template <class T, class... Cols>
struct expression_object_type<
    insert_explicit<std::reference_wrapper<T>, Cols...>> : std::decay<T> {};
...
template <class T, class Ctx>
std::vector<std::string>& collect_table_column_names(
    std::vector<std::string>& collectedExpressions, bool definedOrder,
    const Ctx& context) {
  if (definedOrder) {
    auto& table = pick_table<mapped_type_proxy_t<T>>(context.db_objects);
    collectedExpressions.reserve(collectedExpressions.size() +
                                 table.count_columns_amount());
    table.for_each_column(
        [qualified = !context.skip_table_name, &tableName = table.name,
         &collectedExpressions](const column_identifier& column) {
          if (is_alias_v<T>) {
            collectedExpressions.push_back(
                quote_identifier(alias_extractor<T>::extract()) + "." +
    ...
    if (is_alias_v<T>) {
      ...
      const basic_table& table =
          pick_table<mapped_type_proxy_t<T>>(context.db_objects);
...
struct column_names_getter {
  ...
  std::vector<std::string>& operator()(
      const std::reference_wrapper<T>& expression, const Ctx& context) {
    return (*this)(expression.get(), context);
  }

  template <class T, class Ctx>
  std::vector<std::string>& operator()(const asterisk_t<T>& expression,
                                       const Ctx& context) {
    return collect_table_column_names<T>(collectedExpressions,
                                         expression.defined_order, context);
  }

  template <class T, class Ctx>
  std::vector<std::string>& operator()(const object_t<T>& expression,
                                       const Ctx& context) {
    return collect_table_column_names<T>(collectedExpressions,
                                         expression.defined_order, context);
  }
  ...
  std::vector<std::string>& operator()(const columns_t<Args...>& cols,
                                       const Ctx& context) {
    ...
    // note: `capacity() > size()` can occur in case `asterisk_t<>` does spell
    // out the columns in defined order
...
struct order_by_serializer<order_by_t<O>, void> {
  ...
  template <class Ctx>
  std::string operator()(const statement_type& orderBy,
...
struct statement_serializer<T, match_if<is_bindable, T>> {
 ...
 private:
  template <class X,
            std::enable_if_t<is_printable_v<X> &&
                                 !std::is_base_of<std::string, X>::value
#ifndef SQLITE_ORM_OMITS_CODECVT
  ...
  std::string do_serialize(const std::wstring_view& c) const {
    std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
    return quote_string_literal(
        converter.to_bytes(c.data(), c.data() + c.size()));
  }
#endif
#endif
  /**
...
struct statement_serializer<T, match_specialization_of<T, literal_holder>> {
  ...
  std::string operator()(const T& literal, const Ctx& context) const {
    static_assert(is_bindable_v<type_t<T>>,
                  "A literal value must be also bindable");
...
struct statement_serializer<excluded_t<T>, void> {
  ...
  template <class Ctx>
  std::string operator()(const statement_type& statement,
                         const Ctx& context) const {
    std::stringstream ss;
    ss << "excluded.";
    if (auto* columnName =
            find_column_name(context.db_objects, statement.expression)) {
...
struct statement_serializer<T, match_if<is_upsert_clause, T>> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    if (std::tuple_size<typename statement_type::actions_tuple>::value == 0) {
      ...
      ss << " UPDATE "
         << streaming_actions_tuple(statement.actions, updateContext);
    }
    return ss.str();
  }
};
...
struct statement_serializer<built_in_function_t<R, S, Args...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    ss << statement.serialize() << "("
       << streaming_expressions_tuple(statement.args, context) << ")";
    if (context.use_parentheses) {
      ss << ')';
    }
    return ss.str();
...
struct statement_serializer<as_t<T, E>, void> {
  ...
  std::string operator()(const statement_type& c, const Ctx& context) const {
    ...
    ss << serialize(c.expression, context) + " AS "
       << streaming_identifier(alias_extractor<T>::extract());
    return ss.str();
  }
};
...
template <class E>
struct statement_serializer<
    E, std::enable_if_t<polyfill::disjunction_v<std::is_member_pointer<E>,
                                                is_column_pointer<E>>>> {
    ...
    if (auto* columnName = find_column_name(context.db_objects, e)) {
      ss << streaming_identifier(
          !context.skip_table_name
              ? lookup_table_name<table_type_of_t<E>>(context.db_objects)
              : "",
          *columnName, "");
    } else {
      throw std::system_error{orm_error_code::column_not_found};
...
struct statement_serializer<in_t<L, Args...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    constexpr bool theOnlySelect =
        std::tuple_size<args_type>::value == 1 &&
        is_select_v<std::tuple_element_t<0, args_type>>;
    if (!theOnlySelect) {
      ss << "(";
    }
...
struct statement_serializer<primary_key_t<Cs...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    if (statement.options.conflict_clause_is_on) {
      ss << " ON CONFLICT "
         << serialize(statement.options.conflict_clause, context);
    }
...
template <class... Cs, class... Rs>
struct statement_serializer<foreign_key_t<std::tuple<Cs...>, std::tuple<Rs...>>,
                            void> {
  ...
  std::string operator()(const statement_type& fk, const Ctx& context) const {
    std::stringstream ss;
    ss << "FOREIGN KEY("
       << streaming_mapped_columns_expressions(fk.columns, context)
       << ") REFERENCES ";
    {
      using references_type_t =
          typename std::decay_t<decltype(fk)>::references_type;
      ...
      auto refTableName =
          lookup_table_name<first_reference_mapped_type>(context.db_objects);
...
struct statement_serializer<column_t<G, S, Op...>, void> {
  ...
  template <class Ctx>
  std::string operator()(const statement_type& column,
                         const Ctx& context) const {
    using column_type = statement_type;

    std::stringstream ss;
    ss << streaming_identifier(column.name) << " "
       << type_printer<field_type_t<column_type>>().print() << " "
       << streaming_column_constraints(
              call_as_template_base<column_constraints>(polyfill::identity{})(
                  column),
              column.is_not_null(), context);
    return ss.str();
  }
};
  ...
  std::string operator()(const statement_type& statement,
    ...
    ss << "REPLACE INTO " << streaming_identifier(table.name) << " ("
       ...
       << streaming_field_values_excluding(
              ...
              context, get_ref(statement.object))
       << ")";
    return ss.str();
  }
};
...
struct statement_serializer<insert_explicit<T, Cols...>, void> {
  ...
  std::string operator()(const statement_type& ins, const Ctx& context) const {
    ...
    static_assert(colsCount > 0,
                  "Use insert or replace with 1 argument instead");
    ...
    ss << "("
       << streaming_mapped_columns_expressions(ins.columns.columns, context)
       << ") "
       << "VALUES (";
    iterate_tuple(
        ins.columns.columns, [&ss, &context, &object = get_ref(ins.obj),
                              first = true](auto& memberPointer) mutable {
...
struct statement_serializer<update_t<T>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    table.template for_each_column_excluding<
        ...
        [&table, &ss, &context, &object = get_ref(statement.object),
         first = true](auto& column) mutable {
    ...
    ss << " WHERE ";
    table.for_each_column([&table, &context, &ss,
                           &object = get_ref(statement.object),
                           first = true](auto& column) mutable {
      if (!column.template is<is_primary_key>() &&
          !table.exists_in_composite_primary_key(column)) {
        return;
      }
...
struct statement_serializer<set_t<Args...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    iterate_tuple(statement.assigns, [&ss, &context, &leftContext,
                                      first = true](auto& value) mutable {
      constexpr std::array<const char*, 2> sep = {", ", ""};
      ss << sep[std::exchange(first, false)]
...
std::set<std::pair<std::string, std::string>> collect_table_names(
    const set_t<Args...>& set, const Ctx& ctx) {
...
const std::set<std::pair<std::string, std::string>>& collect_table_names(
    const dynamic_set_t<C>& set, const Ctx&) {
  return set.collector.table_names;
}

template <class Ctx, class T, satisfies<is_select, T> = true>
std::set<std::pair<std::string, std::string>> collect_table_names(
    const T& sel, const Ctx& ctx) {
  auto collector = make_table_name_collector(ctx.db_objects);
...
struct statement_serializer<update_all_t<S, Wargs...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    ss << "UPDATE " << streaming_identifier(tableName) << ' '
       << serialize(statement.set, context)
...
struct statement_serializer<insert_t<T>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    auto& table = pick_table<object_type>(context.db_objects);
    using is_without_rowid =
        typename std::decay_t<decltype(table)>::is_without_rowid;

    std::vector<std::reference_wrapper<const std::string>> columnNames;
    table.template for_each_column_excluding<mpl::conjunction<
        mpl::not_<mpl::always<is_without_rowid>>,
        mpl::disjunction_fn<is_primary_key, is_generated_always>>>(
...
template <class T>
struct statement_serializer<T, std::enable_if_t<polyfill::disjunction_v<
                                   is_insert_raw<T>, is_replace_raw<T>>>> {
  using statement_type = T;
...
struct statement_serializer<remove_t<T, Ids...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    idsStrings.reserve(
        std::tuple_size<typename statement_type::ids_type>::value);
    iterate_tuple(statement.ids, [&idsStrings, &context](auto& idValue) {
      idsStrings.push_back(serialize(idValue, context));
    });
    table.for_each_primary_key_column(
        [&table, &ss, &idsStrings, index = 0](auto& memberPointer) mutable {
          ...
          constexpr std::array<const char*, 2> sep = {" AND ", ""};
          ss << sep[index == 0] << streaming_identifier(*columnName) << " = "
             << idsStrings[index];
          ++index;
        });
    return ss.str();
...
struct statement_serializer<replace_range_t<It, L, O>, void> {
  ...
  std::string operator()(const statement_type& rep, const Ctx& context) const {
    ...
    ss << " VALUES "
       << streaming_values_placeholders(columnsCount, valuesCount);
    return ss.str();
  }
};
...
std::string serialize_get_all_impl(const T& get, const Ctx& context) {
  ...
  ss << "SELECT " << streaming_table_column_names(table, true) << " FROM "
     << streaming_identifiers(tableNames)
     << streaming_conditions_tuple(get.conditions, context);
  return ss.str();
}
...
struct statement_serializer<select_t<T, Args...>, void> {
  ...
  std::string operator()(const statement_type& sel, Ctx context) const {
    ...
    constexpr bool hasExplicitFrom =
        tuple_has<is_from, conditions_tuple>::value;
    if (!hasExplicitFrom) {
      auto tableNames = collect_table_names(sel, context);
      using joins_index_sequence =
          filter_tuple_sequence_t<conditions_tuple, is_constrained_join>;
      // deduplicate table names of constrained join statements
      iterate_tuple(
          sel.conditions, joins_index_sequence{},
          [&tableNames, &context](auto& join) {
            using original_join_type =
                typename std::decay_t<decltype(join)>::type;
...
struct statement_serializer<index_t<T, Cols...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    using indexed_type =
        typename std::decay_t<decltype(statement)>::table_mapped_type;
    ss << "INDEX IF NOT EXISTS " << streaming_identifier(statement.name)
       << " ON "
       << streaming_identifier(
              lookup_table_name<indexed_type>(context.db_objects));
    std::vector<std::string> columnNames;
    std::string whereString;
    iterate_tuple(statement.elements,
                  [&columnNames, &context, &whereString](auto& value) {
                    using value_type = std::decay_t<decltype(value)>;
...
struct statement_serializer<from_t<Args...>, void> {
  ...
  std::string operator()(const statement_type&, const Ctx& context) const {
    ...
    iterate_tuple<tuple>([&context, &ss, first = true](auto* item) mutable {
      ...
      ss << sep[std::exchange(first, false)]
         << streaming_identifier(
                lookup_table_name<mapped_type_proxy_t<from_type>>(
                    context.db_objects),
...
struct statement_serializer<trigger_type_base_t, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    ss << serialize(statement.timing, context) << " "
       << serialize(statement.type, context);
    return ss.str();
  }
};
...
struct statement_serializer<trigger_base_t<T, W, Trigger>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    ss << " ON "
       << streaming_identifier(lookup_table_name<T>(context.db_objects));
...
struct statement_serializer<multi_order_by_t<Args...>, void> {
  ...
  std::string operator()(const statement_type& orderBy,
    ...
    std::stringstream ss;
    ss << static_cast<std::string>(orderBy) << " "
       << streaming_expressions_tuple(orderBy.args, context);
    return ss.str();
  }
};
...
template <class Join>
struct statement_serializer<
    Join, std::enable_if_t<polyfill::disjunction_v<
              polyfill::is_specialization_of<Join, cross_join_t>,
  ...
  std::string operator()(const statement_type& join, const Ctx& context) const {
    ...
    ss << static_cast<std::string>(join) << " "
       << streaming_identifier(
              lookup_table_name<type_t<Join>>(context.db_objects));
    return ss.str();
  }
};
...
struct statement_serializer<Join, match_if<is_constrained_join, Join>> {
  ...
  std::string operator()(const statement_type& join, const Ctx& context) const {
    ...
    ss << static_cast<std::string>(join) << " "
       << streaming_identifier(
              lookup_table_name<mapped_type_proxy_t<type_t<Join>>>(
                  context.db_objects),
...
struct statement_serializer<group_by_with_having<T, Args...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    ss << "GROUP BY " << streaming_expressions_tuple(statement.args, newContext)
       << " HAVING " << serialize(statement.expression, context);
    return ss.str();
  }
};
...
struct statement_serializer<group_by_t<Args...>, void> {
  ...
  std::string operator()(const statement_type& statement,
    ...
    newContext.skip_table_name = false;
    ss << "GROUP BY "
       << streaming_expressions_tuple(statement.args, newContext);
    return ss.str();
  }
};
...
SQLITE_ORM_INLINE_VAR constexpr bool is_preparable_v<
    S, E,
    polyfill::void_t<decltype(std::declval<S>().prepare(std::declval<E>()))>> =
    true;
...
struct storage_t : storage_base {
  ...
  storage_t(std::string filename, db_objects_type dbObjects)
      : storage_base{std::move(filename), foreign_keys_count(dbObjects)},
 ...
 private:
   ...
   *
   *  @note Historically, `serializer_context_builder` was declared friend,
   * along with a few other library stock objects, in order to limit access to
   * the db_objects_tuple. However, one could gain access to a storage_t's
   * db_objects_tuple through `serializer_context_builder`, hence leading the
   * whole friend declaration mambo-jumbo ad absurdum. Providing a free function
   * is way better and cleaner.
   *
   *  Hence, friend was replaced by `obtain_db_objects()` and
   * `pick_const_impl()`.
   */
  friend const db_objects_type& obtain_db_objects(
      const self& storage) noexcept {
   ...
   *  Performs INSERT INTO %destinationTableName% () SELECT %table.column_names%
   * FROM %sourceTableName%
   */
  template <class Table>
  void copy_table(sqlite3* db, const std::string& sourceTableName,
                  const std::string& destinationTableName, const Table& table,
                  const std::vector<const table_xinfo*>& columnsToIgnore) const;

#if SQLITE_VERSION_NUMBER >= 3035000  //  DROP COLUMN feature exists (v3.35.0)
  void drop_column(sqlite3* db, const std::string& tableName,
                   const std::string& columnName) {
    std::stringstream ss;
  ...
  template <class Table>
  void backup_table(sqlite3* db, const Table& table,
                    const std::vector<const table_xinfo*>& columnsToIgnore) {
  ...
  void assert_mapped_type() const {
    ...
    static_assert(
        mpl::invoke_t<check_if_tuple_has_type<O>, mapped_types_tuple>::value,
        "type is not mapped to a storage");
  }

  template <class O, class Table = storage_pick_table_t<O, db_objects_type>,
  ...
  void assert_insertable_type() const {
    ...
    using pkcol_index_sequence =
        col_index_sequence_with<elements_type, is_primary_key>;
    static_assert(
        count_filtered_tuple<elements_type, is_primary_key_insertable,
                             pkcol_index_sequence>::value <= 1,
        "Attempting to execute 'insert' request into an noninsertable table "
        "was detected. "
        "Insertable table cannot contain > 1 primary keys. Please use "
        "'replace' instead of "
        "'insert', or you can use 'insert' with explicit column listing.");
    static_assert(
        count_filtered_tuple<
            elements_type, check_if_not<is_primary_key_insertable>::template fn,
            pkcol_index_sequence>::value == 0,
        "Attempting to execute 'insert' request into an noninsertable table "
        "was detected. "
        "Insertable table cannot contain non-standard primary keys. Please use "
        "'replace' instead "
 ...
 protected:
  ...
  std::string group_concat_internal(F O::*m, std::unique_ptr<std::string> y,
                                    Args&&... args) {
    this->assert_mapped_type<O>();
    std::vector<std::string> rows;
    if (y) {
      rows = this->select(sqlite_orm::group_concat(m, std::move(*y)),
                          std::forward<Args>(args)...);
    } else {
      rows = this->select(sqlite_orm::group_concat(m),
                          std::forward<Args>(args)...);
    }
    if (!rows.empty()) {
 ...
 public:
   ...
   *  throws std::system_error{orm_error_code::not_found} if object not found
   * with given id. throws std::system_error with orm_error_category in case of
   * db error. O is an object type to be extracted. Must be specified
   * explicitly.
   *  @return Object of type O where id is equal parameter passed or throws
   * `std::system_error{orm_error_code::not_found}` if there is no object with
   * such id.
   */
  template <class O, class... Ids>
  O get(Ids... ids) {
   ...
   */
  template <class F, class O, class... Args, class Tuple = std::tuple<Args...>,
  ...
  std::string group_concat(F O::*m, std::string y, Args&&... args) {
    return this->group_concat_internal(
        m, std::make_unique<std::string>(std::move(y)),
        std::forward<Args>(args)...);
  }
  ...
  std::string group_concat(F O::*m, const char* y, Args&&... args) {
    ...
    return this->group_concat_internal(m, std::move(str),
                                       std::forward<Args>(args)...);
  }
   ...
   */
  template <class F, class O, class... Args,
            class Ret = column_result_of_t<db_objects_type, F O::*>>
  ...
  template <class E, class Ex = polyfill::remove_cvref_t<E>,
            std::enable_if_t<!is_prepared_statement_v<Ex> &&
                                 !is_mapped_v<db_objects_type, Ex>,
                             bool> = true>
  std::string dump(E&& expression, bool parametrized = false) const {
    static_assert(is_preparable_v<self, Ex>,
                  "Expression must be a high-level statement");
    ...
    decltype(auto) e2 = static_if<is_select_v<Ex>>(
        ...
        [](const auto& expression) -> decltype(auto) { return (expression); })(
  ...
  void replace_range(It from, It to, Projection project = {}) {
    ...
    auto statement = this->prepare(sqlite_orm::replace_range(
        std::move(from), std::move(to), std::move(project)));
    this->execute(statement);
  }
  ...
  void replace_range(It from, It to, Projection project = {}) {
    ...
    auto statement = this->prepare(sqlite_orm::replace_range<O>(
        std::move(from), std::move(to), std::move(project)));
    this->execute(statement);
  }
  ...
  /**
   *  Raw insert routine. Use this if `insert` with object does not fit you.
   * This insert is designed to be able to call any type of `INSERT` query with
   * no limitations.
   *  @example
   *  ```sql
   *  INSERT INTO users (id, name) VALUES(5, 'Little Mix')
   ...
   *  will be
   *  ```c++
   *  storage.insert(into<User>, columns(&User::id, &User::name),
   ...
   *  will be
   *  ```c++
   *  storage.insert(into<Singer>(), columns(&Singer::name),
   ...
   * IGNORE`/`INSERT OR REPLACE`/`INSERT ROLLBACK`:
   *  ```c++
   *  storage.insert(or_ignore(), into<Singer>(), columns(&Singer::name),
   * values(std::make_tuple("Sofia Reyes"), std::make_tuple("Kungs")));
   *  storage.insert(or_rollback(), into<Singer>(), default_values());
   *  storage.insert(or_abort(), into<User>, columns(&User::id, &User::name),
   * values(std::make_tuple(5, "Little Mix")));
   *  ```
   */
  template <class... Args>
  void insert(Args... args) {
    auto statement =
        this->prepare(sqlite_orm::insert(std::forward<Args>(args)...));
    this->execute(statement);
  }
   ...
   *  will be
   *  ```c++
   *  storage.prepare(replace(into<User>, columns(&User::id, &User::name),
   * values(std::make_tuple(5, "Little Mix"))));
   *  ```
   *  One more example:
   *  ```sql
   ...
   *  will be
   *  ```c++
   *  storage.prepare(replace(into<Singer>(), columns(&Singer::name),
   * values(std::make_tuple("Sofia Reyes"), std::make_tuple("Kungs"))));
   *  ```
   *  One can use `default_values` to add `DEFAULT VALUES` modifier:
   *  ```sql
  ...
  void replace(Args... args) {
    auto statement =
        this->prepare(sqlite_orm::replace(std::forward<Args>(args)...));
    this->execute(statement);
  }
  ...
  void insert_range(It from, It to, Projection project = {}) {
    ...
    auto statement = this->prepare(sqlite_orm::insert_range(
        std::move(from), std::move(to), std::move(project)));
    this->execute(statement);
  }
  ...
  void insert_range(It from, It to, Projection project = {}) {
    ...
    auto statement = this->prepare(sqlite_orm::insert_range<O>(
        std::move(from), std::move(to), std::move(project)));
    this->execute(statement);
  }
  ...
  [[deprecated(
      ...
      "`find_column_name()`")]] const std::string*
 ...
 protected:
  template <class... Cols>
  sync_schema_result schema_status(const index_t<Cols...>&, sqlite3*, bool,
                                   bool*) {
  ...
  sync_schema_result schema_status(const table_t<T, WithoutRowId, Cs...>& table,
    ...
    if (!gottaCreateTable) {
      ...
      if (calculate_remove_add_columns(columnsToAdd, storageTableInfo,
                                       dbTableInfo)) {
        gottaCreateTable = true;
      }

      if (!gottaCreateTable) {  //  if all storage columns are equal to actual
                                //  db columns but there are
        ...
        if (!columnsToAdd.empty()) {
          ...
          for (const table_xinfo* colInfo : columnsToAdd) {
            ...
            if (generatedStorageType) {
              if (*generatedStorageType ==
                  basic_generated_always::storage_type::stored) {
                gottaCreateTable = true;
                break;
              }
              ...
              if (colInfo->notnull && colInfo->dflt_value.empty()) {
                gottaCreateTable = true;
                // no matter if preserve is true or false, there is no way to
                // preserve data, so we wont try!
                if (attempt_to_preserve) {
  ...
  template <class... Cols>
  sync_schema_result sync_table(const index_t<Cols...>& index, sqlite3* db,
                                bool) {
    auto res = sync_schema_result::already_in_sync;
    using context_t = serializer_context<db_objects_type>;
    context_t context{this->db_objects};
  ...
  template <class... Cols>
  sync_schema_result sync_table(const trigger_t<Cols...>& trigger, sqlite3* db,
                                bool) {
  ...
  template <class C>
  void add_column(sqlite3* db, const std::string& tableName,
 ...
 public:
   ...
   * differs by any of properties/constraints (pk, notnull, dflt_value) table
   * will be dropped and recreated. Be aware that `sync_schema` doesn't
   ...
   * `make_storage`, `make_table` and `make_column` calls. The best practice is
   * to call this function right after storage creation.
   *  @param preserve affects function's behaviour in case it is needed to
   * remove a column. If it is `false` so table will be dropped if there is
   * column to remove if SQLite version is < 3.35.0 and remove column if SQLite
   * version >= 3.35.0, if `true` -  table is being copied into another table,
  ...
  std::map<std::string, sync_schema_result> sync_schema(bool preserve = false) {
    ...
    iterate_tuple<true>(this->db_objects, [this, db = con.get(), preserve,
                                           &result](auto& schemaObject) {
   ...
   */
  std::map<std::string, sync_schema_result> sync_schema_simulate(
      bool preserve = false) {
    auto con = this->get_connection();
    std::map<std::string, sync_schema_result> result;
    iterate_tuple<true>(this->db_objects, [this, db = con.get(), preserve,
                                           &result](auto& schemaObject) {
      sync_schema_result status =
          this->schema_status(schemaObject, db, preserve, nullptr);
      result.emplace(schemaObject.name, status);
    });
    return result;
  }

  using storage_base::table_exists;  // now that it is in storage_base make it
                                     // into overload set
  ...
  template <class T, class... Args>
  prepared_statement_t<get_all_t<T, Args...>> prepare(
      get_all_t<T, Args...> get_) {
    return prepare_impl<get_all_t<T, Args...>>(std::move(get_));
  }

  template <class T, class... Args>
  prepared_statement_t<get_all_pointer_t<T, Args...>> prepare(
      get_all_pointer_t<T, Args...> get_) {
    return prepare_impl<get_all_pointer_t<T, Args...>>(std::move(get_));
  }

  template <class... Args>
  prepared_statement_t<replace_raw_t<Args...>> prepare(
      replace_raw_t<Args...> ins) {
    return prepare_impl<replace_raw_t<Args...>>(std::move(ins));
  }

  template <class... Args>
  prepared_statement_t<insert_raw_t<Args...>> prepare(
      insert_raw_t<Args...> ins) {
    return prepare_impl<insert_raw_t<Args...>>(std::move(ins));
  }

#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
  template <class T, class R, class... Args>
  prepared_statement_t<get_all_optional_t<T, R, Args...>> prepare(
      get_all_optional_t<T, R, Args...> get_) {
    return prepare_impl<get_all_optional_t<T, R, Args...>>(std::move(get_));
  }
#endif  // SQLITE_ORM_OPTIONAL_SUPPORTED

  template <class S, class... Wargs>
  prepared_statement_t<update_all_t<S, Wargs...>> prepare(
      update_all_t<S, Wargs...> upd) {
    return prepare_impl<update_all_t<S, Wargs...>>(std::move(upd));
  }

  template <class T, class... Args>
  prepared_statement_t<remove_all_t<T, Args...>> prepare(
      remove_all_t<T, Args...> rem) {
  ...
  template <class T, class... Ids>
  prepared_statement_t<get_pointer_t<T, Ids...>> prepare(
      get_pointer_t<T, Ids...> get_) {
    return prepare_impl<get_pointer_t<T, Ids...>>(std::move(get_));
  }

#ifdef SQLITE_ORM_OPTIONAL_SUPPORTED
  template <class T, class... Ids>
  prepared_statement_t<get_optional_t<T, Ids...>> prepare(
      get_optional_t<T, Ids...> get_) {
  ...
  template <class T, class... Ids>
  prepared_statement_t<remove_t<T, Ids...>> prepare(
      remove_t<T, Ids...> statement) {
    using object_type =
        typename expression_object_type<decltype(statement)>::type;
  ...
  template <class It, class L, class O>
  prepared_statement_t<insert_range_t<It, L, O>> prepare(
      insert_range_t<It, L, O> statement) {
    using object_type =
        typename expression_object_type<decltype(statement)>::type;
    this->assert_mapped_type<object_type>();
    this->assert_insertable_type<object_type>();
  ...
  template <class It, class L, class O>
  prepared_statement_t<replace_range_t<It, L, O>> prepare(
      replace_range_t<It, L, O> statement) {
    using object_type =
        typename expression_object_type<decltype(statement)>::type;
    this->assert_mapped_type<object_type>();
  ...
  template <class T, class... Cols>
  prepared_statement_t<insert_explicit<T, Cols...>> prepare(
      insert_explicit<T, Cols...> ins) {
  ...
  template <class T, class... Cols>
  int64 execute(
      const prepared_statement_t<insert_explicit<T, Cols...>>& statement) {
    ...
    tuple_value_binder{stmt}(
        ...
        [&table = this->get_table<object_type>(),
  ...
  template <class T, std::enable_if_t<polyfill::disjunction_v<
                                          is_replace<T>, is_replace_range<T>>,
                                      bool> = true>
  void execute(const prepared_statement_t<T>& statement) {
    ...
    auto processObject = [&table = this->get_table<object_type>(),
                          bindValue =
                              field_value_binder{stmt}](auto& object) mutable {
      table.template for_each_column_excluding<is_generated_always>(
          call_as_template_base<column_field>(
              [&bindValue, &object](auto& column) {
    ...
    static_if<is_replace_range_v<T>>(
        [&processObject](auto& expression) {
          ...
          std::ranges::for_each(expression.range.first, expression.range.second,
                                std::ref(processObject),
                                std::ref(expression.transformer));
#else
          auto& transformer = expression.transformer;
          std::for_each(expression.range.first, expression.range.second,
                        [&processObject, &transformer](auto& item) {
                          const object_type& object =
                              polyfill::invoke(transformer, item);
  ...
  std::unique_ptr<T> execute(
      const prepared_statement_t<get_pointer_t<T, Ids...>>& statement) {
    sqlite3_stmt* stmt = reset_stmt(statement.stmt);
    ...
    perform_step(stmt,
                 [&table = this->get_table<T>(), &res](sqlite3_stmt* stmt) {
  ...
  std::optional<T> execute(
      const prepared_statement_t<get_optional_t<T, Ids...>>& statement) {
    sqlite3_stmt* stmt = reset_stmt(statement.stmt);
  ...
  template <class T, class... Args>
  void execute(
      const prepared_statement_t<remove_all_t<T, Args...>>& statement) {
    sqlite3_stmt* stmt = reset_stmt(statement.stmt);
  ...
  template <class S, class... Wargs>
  void execute(
      const prepared_statement_t<update_all_t<S, Wargs...>>& statement) {
    sqlite3_stmt* stmt = reset_stmt(statement.stmt);
  ...
  template <class T, class... Args,
            class R = column_result_of_t<db_objects_type, T>>
  std::vector<R> execute(
      const prepared_statement_t<select_t<T, Args...>>& statement) {
    sqlite3_stmt* stmt = reset_stmt(statement.stmt);

    iterate_ast(statement.expression, conditional_binder{stmt});

    std::vector<R> res;
    perform_steps(stmt, [rowExtractor = make_row_extractor<R>(
                             lookup_table<R>(this->db_objects)),
...
/*
 *  Factory function for a storage, from a database file and a bunch of database
 * object definitions.
 */
template <class... DBO>
internal::storage_t<DBO...> make_storage(std::string filename,
                                         DBO... dbObjects) {
 ...
 */
inline int threadsafe() { return sqlite3_threadsafe(); }
}  // namespace sqlite_orm
#pragma once
...
template <class T>
struct node_tuple<T, match_if<is_upsert_clause, T>>
    : node_tuple<typename T::actions_tuple> {};
...
auto& get(internal::prepared_statement_t<internal::insert_range_t<It, L, O>>&
              statement) {
...
template <int N, class It, class L, class O>
const auto& get(
    const internal::prepared_statement_t<internal::insert_range_t<It, L, O>>&
        statement) {
  return std::get<N>(statement.expression.range);
}

template <int N, class It, class L, class O>
auto& get(internal::prepared_statement_t<internal::replace_range_t<It, L, O>>&
              statement) {
  return std::get<N>(statement.expression.range);
}
...
template <int N, class It, class L, class O>
const auto& get(
    const internal::prepared_statement_t<internal::replace_range_t<It, L, O>>&
        statement) {
  return std::get<N>(statement.expression.range);
}
...
auto& get(
    internal::prepared_statement_t<internal::get_t<T, Ids...>>& statement) {
  return internal::get_ref(std::get<N>(statement.expression.ids));
}

template <int N, class T, class... Ids>
const auto& get(
    const internal::prepared_statement_t<internal::get_t<T, Ids...>>&
        statement) {
  return internal::get_ref(std::get<N>(statement.expression.ids));
}
...
template <int N, class T, class... Ids>
const auto& get(
    const internal::prepared_statement_t<internal::get_pointer_t<T, Ids...>>&
        statement) {
  return internal::get_ref(std::get<N>(statement.expression.ids));
}
...
template <int N, class T, class... Ids>
const auto& get(
    const internal::prepared_statement_t<internal::get_optional_t<T, Ids...>>&
        statement) {
  return internal::get_ref(std::get<N>(statement.expression.ids));
}
#endif  // SQLITE_ORM_OPTIONAL_SUPPORTED
...
template <int N, class T, class... Ids>
auto& get(
    internal::prepared_statement_t<internal::remove_t<T, Ids...>>& statement) {
  return internal::get_ref(std::get<N>(statement.expression.ids));
}

template <int N, class T, class... Ids>
const auto& get(
    const internal::prepared_statement_t<internal::remove_t<T, Ids...>>&
        statement) {
  return internal::get_ref(std::get<N>(statement.expression.ids));
}
...
auto& get(internal::prepared_statement_t<internal::update_t<T>>& statement) {
  static_assert(N == 0,
                "get<> works only with 0 argument for update statement");
  return internal::get_ref(statement.expression.object);
}

template <int N, class T>
const auto& get(
    const internal::prepared_statement_t<internal::update_t<T>>& statement) {
  static_assert(N == 0,
                "get<> works only with 0 argument for update statement");
  return internal::get_ref(statement.expression.object);
}
...
auto& get(internal::prepared_statement_t<internal::insert_explicit<T, Cols...>>&
              statement) {
  static_assert(N == 0,
                "get<> works only with 0 argument for insert statement");
  return internal::get_ref(statement.expression.obj);
}

template <int N, class T, class... Cols>
const auto& get(
    const internal::prepared_statement_t<internal::insert_explicit<T, Cols...>>&
        statement) {
  static_assert(N == 0,
                "get<> works only with 0 argument for insert statement");
  return internal::get_ref(statement.expression.obj);
...
auto& get(internal::prepared_statement_t<internal::replace_t<T>>& statement) {
  static_assert(N == 0,
                "get<> works only with 0 argument for replace statement");
  return internal::get_ref(statement.expression.object);
}

template <int N, class T>
const auto& get(
    const internal::prepared_statement_t<internal::replace_t<T>>& statement) {
  static_assert(N == 0,
                "get<> works only with 0 argument for replace statement");
  return internal::get_ref(statement.expression.object);
}
...
template <int N, class T>
const auto& get(
    const internal::prepared_statement_t<internal::insert_t<T>>& statement) {
  static_assert(N == 0,
                "get<> works only with 0 argument for insert statement");
  return internal::get_ref(statement.expression.object);
}
...
const auto& get(const internal::prepared_statement_t<T>& statement) {
  ...
  internal::iterate_ast(statement.expression, [&result,
                                               index = -1](auto& node) mutable {
 ...
 *  which can be achieved before C++17's inline variables, but differs from
 * compiler to compiler. Hence we make it only available for compilers
 * supporting inline variables.
 */
...
/**
 *  Wrap a pointer of type 'carray' and its deleter function for binding it to a
 * statement.
 *
 *  Unless the deleter yields a nullptr 'xDestroy' function the ownership of the
 * pointed-to-object is transferred to the pointer binding, which will delete it
...
auto bindable_carray_pointer(P* p, D d) noexcept
    -> pointer_binding<P, carray_pvt, D> {
 ...
 */
template <class P>
auto statically_bindable_carray_pointer(P* p) noexcept
    -> static_pointer_binding<P, carray_pvt> {
  return statically_bindable_pointer<carray_pvt>(p);
}

/**
 *  Generalized form of the 'remember' SQL function that is a pass-through for
 * values (it returns its argument unchanged using move semantics) but also
 * saves the value that is passed through into a bound variable.
 */
template <typename P>
struct note_value_fn {
  ...
  static constexpr const char* name() { return "note_value"; }
};

/**
 *  remember(V, $PTR) extension function
 * https://sqlite.org/src/file/ext/misc/remember.c
 */
struct remember_fn : note_value_fn<int64> {
  static constexpr const char* name() { return "remember"; }
};
}  // namespace sqlite_orm
#endif
#pragma once
...
inline auto make_dbstat_table() {
  return make_table("dbstat", make_column("name", &dbstat::name),
...
/** @file Mainly existing to disentangle implementation details from circular
 * and cross dependencies (e.g. column_t -> default_value_extractor ->
 * serializer_context -> db_objects_tuple -> table_t -> column_t) this file is
 * also used to provide definitions of interface methods 'hitting the database'.
 */
#pragma once
...
std::unique_ptr<std::string> column_constraints<Op...>::default_value() const {
  using default_op_index_sequence =
      filter_tuple_sequence_t<constraints_type,
                              check_if_is_template<default_t>::template fn>;
  ...
  call_if_constexpr<default_op_index_sequence::size()>(
      [&value](auto& constraints, auto op_index_sequence) {
        ...
        constexpr size_t opIndex =
            first_index_sequence_value(default_op_index_sequence{});
        value = std::make_unique<std::string>(
            serialize_default_value(get<opIndex>(constraints)));
      },
      this->constraints, default_op_index_sequence{});
  return value;
}
...
template <class T, bool WithoutRowId, class... Cs>
std::vector<table_xinfo> table_t<T, WithoutRowId, Cs...>::get_table_info()
    const {
  std::vector<table_xinfo> res;
  res.reserve(
      size_t(filter_tuple_sequence_t<elements_type, is_column>::size()));
  this->for_each_column([&res](auto& column) {
    ...
    res.emplace_back(-1, column.name, type_printer<field_type>().print(),
                     column.is_not_null(), dft,
  ...
  for (size_t i = 0; i < compositeKeyColumnNames.size(); ++i) {
    ...
    auto it = std::find_if(
        res.begin(), res.end(),
        [&columnName](const table_xinfo& ti) { return ti.name == columnName; });
#endif
    if (it != res.end()) {
...
/** @file Mainly existing to disentangle implementation details from circular
 * and cross dependencies this file is also used to separate implementation
 * details from the main header file, e.g. usage of the dbstat table.
 */
...
sync_schema_result storage_t<DBO...>::sync_table(const Table& table,
                                                 sqlite3* db, bool preserve) {
#ifdef SQLITE_ENABLE_DBSTAT_VTAB
  ...
  auto schema_stat =
      this->schema_status(table, db, preserve, &attempt_to_preserve);
  if (schema_stat != sync_schema_result::already_in_sync) {
    if (schema_stat == sync_schema_result::new_table_created) {
      ...
      if (schema_stat == sync_schema_result::old_columns_removed ||
        ...
        //  now get current table info from db using `PRAGMA table_xinfo`
        //  query..
        auto dbTableInfo = this->pragma.table_xinfo(
            table.name);  // should include generated columns

        //  this vector will contain pointers to columns that gotta be added..
        std::vector<const table_xinfo*> columnsToAdd;

        this->calculate_remove_add_columns(columnsToAdd, storageTableInfo,
                                           dbTableInfo);
        ...
        if (schema_stat == sync_schema_result::new_columns_added) {
          for (const table_xinfo* colInfo : columnsToAdd) {
            table.for_each_column(
                [this, colInfo, &tableName = table.name, db](auto& column) {
...
void storage_t<DBO...>::copy_table(
    sqlite3* db, const std::string& sourceTableName,
    const std::string& destinationTableName, const Table& table,
    const std::vector<const table_xinfo*>& columnsToIgnore)
    const {  // must ignore generated columns
  std::vector<std::reference_wrapper<const std::string>> columnNames;
  columnNames.reserve(table.count_columns_amount());
  table.for_each_column(
      [&columnNames, &columnsToIgnore](const column_identifier& column) {
        auto& columnName = column.name;
#if __cpp_lib_ranges >= 201911L
        auto columnToIgnoreIt =
            std::ranges::find(columnsToIgnore, columnName, &table_xinfo::name);
#else
        auto columnToIgnoreIt =
            std::find_if(columnsToIgnore.begin(), columnsToIgnore.end(),
...
#if defined(_MSC_VER)
__pragma(pop_macro("max")) __pragma(pop_macro("min"))
#endif  // defined(_MSC_VER)
